...
Come riferimento e come base per la tesi \`e stato usato il Passenger Counter realizzato da Eurotech.
...
L'applicativo consta di tre parti fondamentali come visibile in figura \ref{fig:SchemPCNEurotech}:
\begin{enumerate}
...
L'obiettivo della tesi \`e stato quello di realizzare una nuova versione del Passenger Counter di Eurotech basandosi sulla infrastruttura software/hardware fornita dall'azienda cercando di migliorare quanto gi\`a fatto dall'azienda, esplorando nuove soluzioni tecnologiche alternative. Gli obiettivi della tesi sono sono stati principalmente tre:
\begin{enumerate}
\item Identificare gli ambienti di sviluppo pi\`u idonei alla realizzazione del progetto.
\item Realizzare una nuova versione del Passenger Counter puntando a migliorare le prestazione ed abbattere i costi di produzione.
\item Realizzare una infrastruttura software che fornisse tutti i tool e le librerie necessarie all'implementazione del Passenger Counter realizzato, che potesse essere installato sulla piattaforma hardware fornita da Eurotech.
\end{enumerate}
\`E quindi possibile accedere ai dati raccolti da pi\`u dispositivi.
