...
Queste operazioni vengono svolte in automatico dai tool resi disponibili dal progetto Yocto, ma \`e lasciato al programmatore il compito di scegliere il software da installare nella distribuzione. Tali preferenze vanno specificate in due file di configurazione del build system:

\begin{itemize}
\item \textbf{bblayers.conf}: contiene i path dei layers. Come spiegato in precedenza i layers sono collezioni logiche di ricette che rappresentano stack di applicazioni o BSP.
\item \textbf{local.conf}: contiene il file di configurazione della build. Qui viene specificato quali software installare nella distribuzione e altre instruzioni circa il processo di build.
\end{itemize}
...
Poich\`e non vi erano problemi di compatibilit\`a fra i layer \`e stato possibile installare l'ultima versione disponibile di tutto il software. Quindi questa in questa versione della distribuzione era disponibile OpenCV 3.2.
...
Lo stack software completo di questa versione della distribuzione \`e riportato in figura \ref{fig:SWStack}.
