...
\subsection{Confronto PCN a sottrazione del background e PCN con telecamere RealSense}
Arrivati a questo punto \`e possibile effettuare un confronto tra le due versioni del Passenger Counter implementate.
...
Il porting dell'applicazione ha comportato subito grossi problemi di compatibilit\`a poich\`e, bench\`e OpenCV avesse introdotto recentemente il supporto a Java, a causa della libreria librealsense \`e stato necessario appoggiarsi al progetto \textbf{JavaCPP}.
...
Nonostante l'overhead computazionale dovuto all'interfacciamento Java, l'ottima parallelizzazione compiuta dalla JVM ha permesso una distribuzione del calcolo su tutti i thread disponibili sul ReliaGate che mitiga questo problema. Il codice cos\`\i\ realizzato quindi ha delle ottime performance anche su macchine poco prestanti.

\textbf{Aggiungere dati sulle performance della versione}

\subsubsection{Versione OSGi}
In pausa finch\`e non siamo sicuri di implementarla.
