...
\begin{lstlisting}[language=C++, caption=RSPCN.cpp metodo count() parte 2, style=customcpp]
        // --FINDING CONTOURS
        findContours(morphTrans, contours, hierarchy, RETR_EXTERNAL, CHAIN_APPROX_NONE);
...
\begin{lstlisting}[language=C++, caption=RSPCN.cpp metodo count() parte 3, style=customcpp]
        // For every passenger in passengers DB
        for(unsigned int i = 0; i < passengers.size(); i++) {
