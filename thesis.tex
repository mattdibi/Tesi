...
Nel seguito sono riportati alcuni fotogrammi tratti da una registrazione di un test del contatore. Per il test \`e stata utilizzata la telecamera descritta nel paragrafo \ref{HardwarePCN}, la quale \`e stata montata sulla traversa di una porta ad una altezza di circa 2,10 m.
...
In ingresso abbiamo il formato immagine illustrato in figura \ref{fig:rawDepthImage} e la soglia espressa in centimetri da applicare all'immagine in ingresso.

Come nel caso precedente applichiamo la funzione \verb|setTo()| per imporre tutti i pixel nulli a 65535. Quindi convertiamo l'immagine da 16 bit privi di segno a 32 bit floating point. Questo passaggio  \`e necessario per poter passare l'immagine alla funzione \verb|threshold()|, la quale accetta solamente immagini nel formato 8 bit privi di segno o 32 bit floating point. Il risultato di questo passaggio \`e visibile in figura \ref{fig:getFrameStep1}.
...
A questo punto passiamo a rilevare i contorni degli oggetti in primo piano tramite la funzione \verb|findCountours()|. Per ogni oggetto cos\`\i\ ricavato ne calcoliamo l'area, se l'area supera un valore minimo deciso arbitrariamente in fase sperimentale viene considerato un passeggero.
...
\item Si rilevano gli attraversamenti della soglia:
    \begin{itemize}
    \item Se l'ultima posizione si trova al di sotto della soglia e la nuova posizione rilevata al di sopra, si aggiorna il contatore in ingresso. 
...
Come nella versione precedente si \`e deciso di plottare a schermo tutte le informazioni utili a semplificare la comprensione del funzionamento del programma e al debugging. Un esempio della schermata tipica durante il funzionamento del contatore \`e riportato in figura \ref{fig:RSPCN}.
...
Nel seguito sono riportati alcuni fotogrammi tratti da una registrazione di un test del contatore effettuata utilizzando la telecamera Intel RealSense SR300, la quale \`e stata montata su uno stipite di una porta ad una altezza di circa 2,10 m. Il formato dei frame \`e lo stesso del caso precedente.
...
Si noti che a differenza del caso precedente nell'immagine contenente i dati di profondit\`a non viene rilevato il pavimento. Questo \`e dovuto al fatto che il range operativo della telecamera SR300 \`e inferiore a quello della R200. Ci\`o per\`o non influisce sul corretto funzionamento del contatore.
