...
L'approccio modulare permette alle funzioni contenute nei vari moduli di essere riutilizzate negli altri per ottenere un approccio pi\`u ad alto livello. Le funzioni viste in un modulo sono spesso utilizzate in congiunzione e come supporto degli altri moduli. Un esempio tipico \`e l'applicazione del filtro Sobel (modulo imgproc) per la rilevazione dei contorni delle figure per aiutare il rilevamento di oggetti (modulo objdetect). Un altro esempio \`e l'applicazione di filtri per il pre-processing di un training set prima di essere dato in pasto all'algoritmo di machine learning per l'estrazione di feature significative che migliorino l'apprendimento.

%% Interruzione di pagina
\newpage

%% Copiato spudoratamente dalla mia presentazione su Yocto.
%% TODO: Migliorare il contenuto di questa sezione
\subsection{Yocto Project}\label{Yocto}
Yocto Project \`e un insieme di strumenti che vanno a creare immagini di sistemi operativi per sistemi Linux
embedded corredati da toolchain di crosscompilazione, emulatori ecc ecc. Non \`e una distribuzione Linux
embedded, ma permette di crearne una custom per la propria piattaforma hardware.
Yocto Project \`e quindi un ombrello sotto il quale si raccolgo vari sotto-progetti volti allo sviluppo di sistemi
Linux Embedded.
...
\item Poky: \`e una reference distribution di Yocto Project. Contiene l'OpenEmbedded Build System(BitBake e OpenEmbedded Core) assieme a un insieme di metadati che permettono di cominciare a costruire la propria distribuzione Linux embedded customizzata. In pratica fa da distribuzione template.
\end{itemize}
...
Nella fase iniziale del progetto si \`e studiato quale potesse essere l'ambiente di sviluppo adatto all'applicazione.
