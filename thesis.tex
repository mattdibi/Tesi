...
Questa directory include una copia di BitBake per facilitare l'uso del build system. Solitamente questa copia \`e riferita alla versione pi\`u recente di BitBake. BitBake \`e un interprete di metadati, legge i metadati del progetto Yocto ed esegue le task definite da quei dati. Quando viene eseguito il comando \verb|bitbake| che lancia il processo di build, viene chiamato l'eseguibile principale di BitBake, il quale risiede nella cartella \url{bitbake/bin/}. 
...
Lo script ricava la lista dei defalut target dal file conf-notes.txt, il quale si trova nella directory \url{meta-yocto/}.
...
\item \verb|PARALLEL_MAKE|: Opzioni extra passate al comando \verb|make| durante la task \verb|do_compile| in modo tale da specificare la compilazione parallela sulla macchina del build system. Questa variabile solitamente \`e nella forma ``-j x'', dove x rappresenta il numero massimo di thread paralleli che il comando make pu\`o istanziare.
...
La keyword \verb|require| indica quale ricetta \`e necessaria per l'installazione della libreria.
