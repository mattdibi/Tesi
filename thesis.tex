...
\subsection{Funzionamento Passenger Counter con background subtraction}
Nel seguito tratteremo il funzionamento della prima implementazione del contatore. Suddivideremo il codice in tre parti:
\begin{enumerate}
\item Rilevamento dei blob
\item Tracking dei blob
\item Conteggio degli attraversamenti
\end{enumerate}

\subsubsection{Rilevamento dei blob}
...
\begin{lstlisting}[language=C++, caption=Codice rilevamento dei blob]
VideoCapture cap;
...
\item I frame vengono passati alla funzione che applica la sottrazione del background.
\item Seguono delle trasformazioni morfologiche sui frame necessarie a minimizzare il rumore nell'immagine e semplificare il lavoro della funzione di rilevazione dei contorni.
\item Quindi i frame vengono passati alla funzione findContours la quale ricava i contorni delle immagini che le vengono passate. Questi contorni costituiscono i blob che rappresentano i passeggeri che si muovo al di sotto del rilevatore.
\end{enumerate}

Vediamo ora nel dettaglio cosa fanno le funzioni di cui sopra.

% TODO: Tradurre e dare un significato al paragrafo seguente
\subsubsection{Sottrazione del background}
It is also a Gaussian Mixture-based Background/Foreground Segmentation Algorithm. It is based on two papers by Z.Zivkovic, "Improved adaptive Gausian mixture model for background subtraction" in 2004 and "Efficient Adaptive Density Estimation per Image Pixel for the Task of Background Subtraction" in 2006. It uses a method to model each background pixel by a mixture of K Gaussian distributions (K = 3 to 5). The weights of the mixture represent the time proportions that those colours stay in the scene. The probable background colours are the ones which stay longer and more static. One important feature of this algorithm is that it selects the appropriate number of gaussian distribution for each pixel. (Remember, in last case, we took a K gaussian distributions throughout the algorithm). It provides better adaptibility to varying scenes due illumination changes etc.
...
\subsubsection{Trasformazioni morfologiche}

\subsubsection{Rilevamento dei contorni}
