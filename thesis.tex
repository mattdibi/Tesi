...
\begin{lstlisting}[language=C++, caption=PNC.cpp metodo count() parte 2, style=customcpp]
while(!halt)
{
    ...
...
\begin{lstlisting}[language=C++, caption=PNC.cpp metodo count() parte 3, style=customcpp]
if(areaCurrentObject > areaMin)
{
    // Getting bounding rectangle
    Rect br = boundingRect(contours[idx]);
...
\begin{lstlisting}[language=C++, caption=PNC.cpp metodo count() parte 4, style=customcpp]
...
\begin{lstlisting}[language=C++, caption=RSPCN.cpp metodo count(), style=customcpp]
void RSPCN::count()
{
    // Streams
    Mat frame;
...
Questa directoru contiene i sorgenti della documentazione del progetto Yocto e template e tootl necessari a generare le versioni PDF e HTML dei manuali. Ogni manuale \`e contenuto in una sottocartella.
...
\item \verb|PARALLEL_MAKE|: Opzioni extra passate al comando \verb|make| durante la task \verb|do_compile| task in modo tale da specificare la compilazione parallela sulla macchina del build system. Questa variabile solitamente \`e nella forma "-j x", dove x rappresenta il numero massimo di thread paralleli che il comando make pu\`o instanziare.
...
Anche in questo caso, all'avvio, viene instanziato un contatore per telecamera collegata.
...
L'ultima modalit\`a di esecuzione si \`e resa necessaria poich\`e, essendo le fineste di questa versione basate sul framework Java e non su X, impediscono il corretto funzionamento del programma sul ReliaGate 20-25.
...
\begin{enumerate}
\item \textbf{Difficolt\`a nel reperire l'hardware}: le telecamere RealSense sono un dispositivo di nuova concezione e da poco sul mercato. La Intel sembra intenzionata a fornire supporto a questa tencologia a lungo e quindi non sembra possano insorgere problemi di reperibilit\`a a breve termine.
