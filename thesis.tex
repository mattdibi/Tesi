...
Purtroppo, avendo la scheda una sola porta HDMI disponibile solo in output, \`e stato necessario procurarsi un modulo aggiuntivo da collegarsi all'header FMC per avere a disposizione una ulteriore porta HDMI in ingresso. Questo modulo \`e il AES-FMC-HDMI-CAM-G HDMI Input/Output FMC prodotto dalla Avnet\cite{avnet:moduloHDMI}.
...
Nel seguito sono riportati i parametri di impostazione della funzione che implementa l'algoritmo di sottrazione del background:
    ...
    \item \verb|varThreshold|: soglia sulla distanza quadratica di Mahalanobis tra pixel e il modello dello sfondo, atta a discriminare qualora un pixel sia ben descritto dal modello. In caso negativo il pixel viene attribuito al foreground. Si \`e lasciato il valore di default associato a 4 sigma.
...
La differenza nei tempi di esecuzione nei due test \`e dovuta al fatto che nel caso della telecamera R200 si \`e usata una risoluzione pi\`u bassa e un framerate pi\`u elevato del caso della RealSense SR300.
