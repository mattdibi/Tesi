...
\item \textbf{main}: implementa l'interfaccia utente per gestire i settaggi run-time dei PCN.
\end{enumerate}

%% Interruzione di pagina
\newpage

\subsubsection{Classe Passenger}
...
TODO: Sistemare da qui in poi. Aggiungere
\begin{itemize}
\item Finalit\`a della classe
\item Analisi dell'header
\item Quindi passare a descrivere l'algoritmo vero e proprio
\end{itemize}

%% Interruzione di pagina
\newpage

% TODO: Da qui in poi aggiornare tutto il codice e le immagini
\subsection{Analisi del kernel del programma}
...
if(areaCurrentObject > areaMin)
{
    ...
    // If wasn't near any known object it is a new passenger
    if(newPassenger)
    {
        Passenger p(pid, mc);
...
Si noti che oltre a disegnare le tracce dei passeggeri, il ciclo for viene usato anche per aggiornare il parametro age dei passeggeri. Quando questo parametro supera un certo valore di soglia, dovuto al fatto che i passeggeri sono usciti dal raggio visivo della telecamera da troppo tempo, vengono rimossi dal database.
...
%TODO: Parlare degli FPS e tempo di esecuzione
