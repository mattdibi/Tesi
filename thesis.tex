...
\item Progettazione e implementazione del contatore sfruttando telecamere ad infrarossi Intel RealSense. Nel resto del testo ci riferiremo a questa versione dell'applicazione come: Passenger Counter con telecamere RealSense, o RPC.
\end{itemize}
...
L'algoritmo \`e descritto in Suzuki, S. and Abe, K., \textit{Topological Structural Analysis of Digitized Binary Images by Border Following}, CVGIP 30 1, pp 32-46 del 1985.

\subsection{Principio di funzionamento}
Alla base dell'algoritmo implementato c'\`e l'algoritmo noto come \textit{Freeman Chain Code of Eight Directions} (FCCE). L'algoritmo consiste nel codificare i poligoni presenti in una immagine come una sequenza di step in una delle 8 direzioni rappresentate in figura \ref{fig:Freeman}, in modo tale che ogni componente connesso o "blob" sia codificato separatamente. Ogni step, successivo al primo, viene quindi rappresentato da un numero intero compreso tra 0 e 7.
...
Si scannerizza l'immagine per individuare il punto dal quale far partire la sequenza che descrive il poligono. Il punto di partenza \`e rappresentato dalle coordinate del pixel all'interno dell'immagine. 
