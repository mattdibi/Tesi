...
\item Implementare il ``nucleo'' del programma, ovvero la parte di codice incaricata di effettuare il conteggio degli attraversamenti della soglia da parte dei passeggeri sfruttando gli algoritmi di cui abbiamo parlato in precedenza.
...
\item \verb|count()|: \`e il nucleo del programma. Al suo interno avvengono le operazioni di elaborazioni delle immagini e il conteggio degli attraversamenti. Questo metodo verr\`a analizzato separatamente nel prossimo paragrafo.
\end{itemize}
...
\item Per ogni contorno completo rilevato dalla funzione eseguiamo le seguenti operazioni:
    \begin{enumerate}
    \item Ne calcoliamo l'area per mezzo della funzione OpenCV \verb|contourArea()|. Se supera una certa area minima (\verb|areaMin|) decidiamo che si tratta di una persona.
    \item Ne ricaviamo il rettangolo che inscrive i contorni tramite la funzione \verb|boundingRectangle|.
    \item Di questo rettangolo rileviamo il centro \verb|objCenter| grazie ad una semplice operazione geometrica.
    \item Per semplificare lo sviluppo e il debugging andiamo a disegnare questo rettangolo e il rispettivo centro sull'immagine a colori per mezzo delle funzioni \verb|rectangle()| e \verb|circle()| rese disponibili da OpenCV.
    \end{enumerate}
\end{enumerate}
...
\item \textbf{Tracciamenti indesiderati}: Come visto nel paragrafo precedente, nonostante l'algoritmo riesca a rilevare la maggior parte delle ombre come tali, spesso queste vengono comunque tracciate erroneamente. Ci\`o comporta errori nel conteggio.
