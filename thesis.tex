...
Nonostante usino tecnologie diverse entrambi i modelli di telecamera forniscono lo stesso formato in uscita. Lo stream di profondit\`a \`e uno stream di immagini a 16 bit privi di segno in cui il valore di ogni pixel rappresenta la distanza dalla telecamera. La distanza \`e fornita a meno di un fattore di scala reperibile per mezzo di una semplice chiamata di funzione. \`E possibile quindi risalire alla distanza in metri di un pixel dalla telecamera semplicemente moltiplicando il valore del pixel per il fattore di scala.
...
Siccome una parte centrale del progetto verteva sull'elaborazione delle immagini \`e stato necessario integrare nel sistema l'uso di OpenCV, ormai standard de facto nell'ambito dell'elaborazione delle immagini.
...
\end{itemize}
Questa struttura \`e la stessa per tutti i tipi di immagine, indipendentemente dal formato nella quale \`e salvata. Questa astrazione facilita notevolmente la manipolazione delle immagini in quanto ci si riconduce velocemente ad una matrice di pixel manipolabile. Questa conversione \`e garantita dal modulo imgcodecs, la quale funzione non \`e altro che recuperare i dati sui pixel dell'immagine sulla quale si sta lavorando e generare i metadati per il tipo \verb|Mat|.
...
Il porting dell'applicazione ha comportato fin da subito dei problemi di compatibilit\`a dato che, bench\'e OpenCV avesse recentemente introdotto il support a Java, \`e stato necessario utilizzare un wrapper third-party di OpenCV poich\'e lo richiedeva il wrapper ufficiale della libreria librealsense. Entrambi questi wrapper sono stati realizzati dal progetto \textbf{JavaCPP}\footnote{Repository del progetto all'indirizzo: https://github.com/bytedeco/javacpp.}.
