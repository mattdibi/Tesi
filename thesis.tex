...
\usepackage{capt-of}
\graphicspath{ {img/Introduzione/} {img/ContestoTecnologico/1_PCNEurotech/} {img/ContestoTecnologico/2_TecnTesi/} {img/CorpoTesi/1_AmbientiLavoro/} {img/CorpoTesi/2_PCNBackSub/} {img/CorpoTesi/3_RSPCN/} }
...
\subsection{Confronto versioni Passenger Counter}
Arrivati a questo punto \`e possibile effettuare un confronto tra le varie versioni implementate.

Anzitutto \`e necessario considerare i vantaggi della versione con telecamere RealSense rispetto alla versione a sottrazione del background. Grazie alla nuova versione abbiamo ricavato i seguenti vantaggi:
\begin{itemize}
\item \textbf{Indipendenza dalle condizioni di luce}: la versione a sottrazione del background aveva una forte dipendenza dalle condizioni di luce. Variazioni troppo brusche nelle condizioni di illuminazione ne compromettono il funzionamento, essendo basato su un modello dello sfondo costante.
\item \textbf{Minor numero di falsi positivi}: nell'implementazione con le telecamere a infrarossi le ombre non vengono pi\`u considerate come oggetto in movimento da tracciare.
\item \textbf{Costo computazionale}: l'algoritmo di sottrazione del background oltre ad essere impreciso \`e molto pesante dal punto di vista computazionale. L'aver spostato gran parte del lavoro sugli ASIC presenti nelle telecamere permette di avere uno stream in ingresso con un framerate molto pi\`u stabile ed elevato, inoltre permette di utilizzare molte pi\`u telecamere contemporaneamente.
\item \textbf{Precisione}: le prestazioni del Passenger Counter a sottrazione del background sono di gran lunga peggiori dell'implementazione successiva. Il numero di falsi positivi nel tracciamento \`e molto maggiore del numero di oggetti tracciati correttamenti.
\item \textbf{Indipendenza dall'ambiente}: l'algoritmo di sottrazione del background basandosi sulla costruzione di un modello del backgrond costante necessita che la telecamera sia montata su una struttura che non sia soggetta a vibrazioni. Nell'applicazione del sistema in un mezzo di trasporto ci\`o non \`e possibile.
\end{itemize}

\begin{figure}[h!]
  \centering
  \includegraphics[width=\textwidth]{TrackRSPCN.png}
  \caption{Dettaglio tracciamento con Passenger Counter con telecamere RealSense.}
  \label{fig:TrackRSPCN}
\end{figure}

Gli unici svantaggi di questa implementazione sono:
\begin{itemize}
\item \textbf{Raggio di azione}: l'implementazione con sottrazione del background non ha problemi legati al range di funzionamento.
\item \textbf{Costi}: il contatore con sottrazione dello sfondo non \`e vasato su nessun tipo di tecnologia particolare e pu\`o essere implementato con qualsiasi tipo di telecamera. Dal punto di vista dei costi di produzione questo \`e un grosso vantaggio.
\end{itemize}

%% Interruzione di pagina
\newpage

\subsection{Versioni realizzate}
Nel corso della tesi sono state realizzate molteplici versioni dello stesso codice. Il motivo principale per questi porting \`e stato quello di adattarsi al linguaggio di programmazione utilizzato maggiormente in Eurotech.

\subsubsection{Versione C++}
Il codice analizzato finora \`e riferito a questa versione del codice. Siccome OpenCV e la libreria RealSense sono scritte in questo linguaggio si \`e pensato di usare questo come punto di partenza. In seguito alla prima implementazione si \`e reso necessario il porting negli altri linguaggi.

\subsubsection{Wrapping in Java}
Come primo passo si \`e deciso di effettuare un wrapping in Java del codice nativo sfruttando il tool \textbf{SWIG}.
...
Il porting dell'applicazione ha comportato subito grossi problemi di compatibilit\`a poich\`e, bench\`e OpenCV avesse built-in l'interfacciamento a Java, per quanto riguarda la libreria librealsense \`e stato necessario appoggiarsi al progetto \textbf{JavaCPP}.

% TODO: Tradurre e dargli un senso
...
Il progetto rende disponibile i file \verb|.jar| per l'interfacciamento alle librerie librealsense. Purtroppo questa interfaccia fa uso del progetto JavaCV, il quale \`e un porting Java di OpenCV realizzato sempre dal progetto JavaCPP. \`E stato quindi necessario utilizzare JavaCV al posto di OpenCV per realizzare questa versione del codice nonostante quest'ultimo disponesse di un suo interfacciamento a Java. \`E necessario notare per\`o che il progetto JavaCV \`e pi\`u vecchio dell'attuale interfacciamento ufficiale a Java di OpenCV e molto pi\`u completo della controparte. Nonostante ci\`o le differenze tra i due erano cos\`\i\ spiccate che \`e stata necessaria una revisione abbastanza corposa del codice e in alcuni casi non \`e stato possibile portare tutte le feature presenti nella versione C++.

Caratteristiche di questa implementazione:
\begin{itemize}
\item Utilizzo del tipo IplImage al posto di Mat per la rappresentazione dei frame
\item Assenza colorMap
