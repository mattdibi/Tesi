...
%% Dedica (opzionale)
\begin{dedication}A mia madre e ai miei nonni,

per avermi supportato (e sopportato) in questi lunghi anni di studio.\end{dedication}
...
%% Sommario (opzionale)
\abstract
Nello sviluppo di questa tesi si \`e affrontato lo studio e la progettazione di un sistema di conteggio di persone, chiamato ``Passenger Counter'', su piattaforma embedded. Il software \`e basato su algoritmi di elaborazione di immagine resi disponibili dalla libreria open-source per la computer vision OpenCV. La piattaforma software \`e stata realizzata utilizzando il progetto Yocto, il quale permette la creazione di distribuzioni Linux custom finalizzate all'utilizzo nell'ambito dei sistemi immersi.
...
\item Progettazione e implementazione del contatore sfruttando le telecamere ad infrarossi Intel RealSense. Nel resto del testo ci riferiremo a questa versione dell'applicazione come: Passenger Counter con telecamere RealSense, o RSPCN.
\end{itemize}
...
Un altro settore nel quale si proiettano ulteriori sviluppi \`e la Big Data Analysis. L'ubiquit\`a dei dispositivi intelligenti connessi all'Internet delle Cose permette la raccolta di un vastissimo quantitativo di dati ai quali precedentemente era impensabile avere accesso. Le informazioni che si possono ricavare da questi dati sono molteplici e di sicuro interesse per molti ambiti di applicazione.
...
Le applicazioni di monitoraggio ambientale tipicamente utilizzano sensori collegati all'Internet delle Cose per fornire assistenza nella protezione dell'ambiente. Il monitoraggio pu\`o interessare la qualit\`a dell'aria o dell'acqua, condizioni atmosferiche o del suolo, e pu\`o includere aree come il monitoraggio degli spostamenti della fauna selvatica e del suo habitat.

Ci\`o pu\`o avere applicazioni anche nell'ambito del rilevamento di disastri naturali: sistemi di allerta per terremoti e tsunami possono essere implementati nell'ambito IoT. I dispositivi in questo campo di applicazione sono tipicamente dislocati su un'ampia area geografica e possono anche essere mobili.
...
L'integrazione di reti di sensori e attuatori, connessi ad internet, si pensa possa ottimizzare il consumo energetico in ambito industriale e casalingo. L'integrazione di device IoT all'interno di contatori per l'energia, cos\`\i\ come dispositivi domestici e industriali, capaci di comunicare con le compagnie per la rete elettrica possono permettere una gestione migliore della generazione e utilizzo dell'energia. Questo tipo di device inoltre consente agli utenti di regolare in remoto i loro dispositivi, o controllarli centralmente grazie a interfacce web accessibili tramite la rete cloud IoT, in modo tale da implementare funzioni avanzate come la programmazione delle accensioni/spegnimenti o dell'utilizzo dei dispositivi. Uno dei prodotti consumer pi\`u comuni in questo ambito \`e il termostato IoT\cite{Thermostat}, diffuso soprattutto in applicazioni domotiche.

\subsection{Settore medicale e assistenza sanitaria}
Dispositivi connessi all'Internet delle Cose possono abilitare il monitoraggio remoto dello stato di salute dei pazienti e sistemi di notifica delle emergenze. Le applicazioni di queste device possono variare dal rilevamento della pressione arteriosa fino al conteggio dei battiti del cuore. Alcuni ospedali hanno cominciato ad utilizzare ``letti smart'' in grado di rilevare quando il letto \`e occupato e quando il paziente sta cercando di alzarsi. 

Ormai sono molto diffuse anche device per il tracciamento delle attivit\`a dell'utente che incoraggiano uno stile di vita salutare, in questo ambito rientrano i dispositivi indossabili come i fitness trackers.

\subsection{Settore dei trasporti}
L'IoT pu\`o dare assistenza nella integrazione delle comunicazioni, controlli ed elaborazione delle informazioni attraverso vari sistemi di trasporto. Le applicazioni dell'IoT si estendono a tutti gli aspetti dei sistemi di trasporto: i veicoli, l'infrastruttura, il pilota e i passeggeri. L'interazione dinamica tra questi componenti permette una comunicazione inter e intra veicolare, controllo del traffico intelligente, smart parking, gestione della logistica e delle flotte, controllo dei veicoli e sicurezza stradale.
...
L'obiettivo della tesi \`e stato quello di realizzare una nuova versione del Passenger Counter di Eurotech basandosi sull'hardware fornito dall'azienda, cercando di migliorare quanto gi\`a fatto dalla stessa ed esplorando soluzioni tecnologiche alternative. Il lavoro \`e stato quindi suddiviso in tre parti:
\begin{enumerate}
\item Identificare gli ambienti di sviluppo pi\`u idonei alla realizzazione del progetto.
\item Realizzare una nuova versione del Passenger Counter puntando a migliorare le prestazione e abbattere i costi, che fosse compatibile con le tecnologie usate dall'azienda per i suoi prodotti. 
...
\item Capitolo \ref{CapitoloYocto}: Realizzazione e features della distribuzione Linux a supporto del progetto.
...
Il gateway \`e un prodotto della Eurotech noto come ReliaGate 50-21. Utilizza un processore x86 Intel Atom Z510P al quale sono state aggiunte opportune connessioni di rete per il deployment mobile. Esso \`e dotato infatti di interfacce 2G/3G, WiFi, 802.15.4/Zigbee e GPS\cite{eurotech:ReliaGate50-21}. Questo componente rappresenta l'unit\`a di elaborazione centrale del sistema nonch\'e il mezzo attraverso il quale i dati di conteggio vengono raccolti e trasmessi alla piattaforma cloud di Eurotech.
...
Il funzionamento \`e il seguente: i proiettori di luce infrarossa illuminano la scena, le due telecamere acquisiscono le immagini nello spettro della luce infrarossa. Queste vengono passate alla FPGA che, per mezzo di tecniche di ricostruzione stereoscopica\footnote{Per approfondire il funzionamento della Computer Stereo Vision si faccia riferimento all'appendice \ref{ComputerStereoVision} a pagina \pageref{ComputerStereoVision}.} accelerate in hardware, permette di generare una immagine in scala di grigi dove l'informazione di profondit\`a \`e data dal colore dei pixel. Queste immagini vengono quindi passate all'unit\`a di elaborazione centrale che vi applica un semplice algoritmo per il tracciamento dei passeggeri di cui discuteremo il funzionamento nel prossimo paragrafo.
...
La piattaforma a supporto dell'applicazione del Passenger Counter Eurotech \`e basata su una distribuzione custom di Linux realizzata per mezzo del progetto Yocto\footnote{Per approfondimenti circa questo strumento si veda il paragrafo \ref{Yocto} a pagina \pageref{Yocto}.}. Ad essa sono stati aggiunti i driver proprietari per le tecnologie Eurotech. Al di sopra di essa \`e presente la Java Virtual Machine la quale permette l'esecuzione del framework OSGi. 

OSGi (Open Service Gateway Initiative) \`e un framework Java per lo sviluppo e il deployment di programmi software e librerie modulari. Applicazioni e componenti, i quali sono raccolti in ``bundle'' per il deployment, possono essere installati, fermati, aggiornati e disinstallati in remoto senza che sia necessario il reboot del sistema.
...
Nei prossimi capitoli vedremo come sia stato necessario adattare l'applicazione realizzata nel corso di questa tesi alla infrastruttura qui descritta, nonch\'e generare da zero una nuova distribuzione Linux per aggiungere funzionalit\`a mancanti e necessarie alla nuova versione del Passenger Counter.
...
In questa sezione andremo ad esaminare le tecnologie che sono state utilizzate per realizzare la nuova versione del Passenger Counter. Si tenga presente che le telecamere RealSense sono state usate solamente nella seconda implementazione del Passenger Counter: la versione RSPCN.
...
Il principio di funzionamento di questo tipo di telecamere \`e lo stesso del Passenger Counter Eurotech. Sul modulo sono presenti due proiettori ad infrarossi che illuminano la scena, le due telecamere stereoscopiche acquisiscono due immagini leggermente diverse. Analizzando le differenze tra le due immagini per mezzo di algoritmi di image processing viene ricostruita l'informazione di profondit\`a\footnote{Per approfondire il funzionamento della Computer Stereo Vision si faccia riferimento all'appendice \ref{ComputerStereoVision} a pagina \pageref{ComputerStereoVision}.}. Per ottenere una ricostruzione delle informazioni in real-time \`e stato usato un circuito ASIC (Application-Specific Integrated Circuit) che garantisse le prestazioni desiderate. Il funzionamento \`e riassunto in figura \ref{fig:SchemaFunzR200}.
...
Per l'interfacciamento software la Intel fornisce una libreria in C++ tramite la quale \`e possibile recuperare i frame e i dati di profondit\`a dalle telecamere: la libreria \textbf{librealsense}\footnote{Repository libreria all'indirizzo: \url{https://github.com/IntelRealSense/librealsense}.}. Essa provvede all'inizializzazione delle telecamere, apertura dei vari stream video disponibili, impostazione di risoluzione e framerate. Gli stream disponibili sono tre: 

\begin{enumerate}
\item Lo stream proveniente dalla telecamera a colori.
\item Il flusso video in output dalle telecamere ad infrarossi prima di essere processato per ricavare le informazioni di profondit\`a.
\item I frame di profondit\`a provenienti dall'ASIC contenenti le immagini in cui \`e codificata l'informazione di distanza.
\end{enumerate}

Nonostante usino tecnologie diverse entrambi i modelli di telecamera forniscono lo stesso formato in uscita. Lo stream di profondit\`a \`e un flusso di immagini a 16 bit privi di segno in cui il valore di ogni pixel rappresenta la distanza dalla telecamera. La distanza \`e fornita a meno di un fattore di scala reperibile per mezzo di una semplice chiamata di funzione. \`E possibile quindi risalire alla distanza in metri di un pixel dalla telecamera semplicemente moltiplicando il valore del pixel per il fattore di scala.
...
\end{itemize}
Questo fatto non ha per\`o comportato problemi dal punto di vista della funzionalit\`a dell'applicazione in quanto la risoluzione delle telecamere non \`e critica per il corretto funzionamento del contatore e il fattore di scala viene ricavato run-time.

%% Copiato spudoratamente dalla mia presentazione su OpenCV.
\subsection{Libreria per l'image processing: OpenCV}\label{OpenCV}
Siccome la parte centrale del progetto verteva sull'elaborazione delle immagini \`e stato necessario integrare nel sistema l'uso di OpenCV, ormai standard de facto nell'ambito dell'image processing.

OpenCV, acronimo di Open Source Computer Vision, \`e una libreria software multi piattaforma finalizzata all'image processing real-time e alla computer vision. Essa \`e rilasciata tramite licenza Berkeley Software Distribution (BSD) quindi \`e ad uso gratuito sia per fini accademici che commerciali e contiene pi\`u di 2500 algoritmi pre-ottimizzati per le operazioni di elaborazione di immagini, computer vision e machine learning pi\`u comuni. Sviluppata in C/C++, \`e dotata di interfacce verso C, Python, Java e MATLAB. Poich\'e finalizzata all'utilizzo real-time, la libreria sfrutta molteplici interfacce per l'accelerazione hardware (CUDA, OpenCL, Intel Integrated Perfomance Primitives). OpenCV ha una struttura modulare, il che significa che il pacchetto include diverse librerie statiche o condivise.

\subsubsection{Moduli principali e finalit\`a}
\begin{itemize}
\item \textbf{Modulo core}: lo scopo del modulo \`e  definire interfacce e funzionalit\`a che permettano di semplificare la manipolazione di immagini e flussi video. Il modulo contiene le funzioni alla base delle libreria e ne definisce le strutture fondamentali nonch\'e gestisce la memoria. 
\item \textbf{Modulo highui}: High-level GUI e Media I/O. Il modulo HighGUI \`e stato progettato per fornire funzioni che permettano di provare le feature della libreria e osservare i risultati velocemente. Fornisce semplici interfacce per creare e manipolare finestre che possano visualizzare immagini e aggiungervi slider, gestire semplici eventi come click del mouse e comandi da tastiera.
\item \textbf{Modulo imgproc}: image processing. Il modulo di Image Processing contiene funzioni e classi per l'elaborazione di immagini. Ci\`o comprende:
    \begin{itemize}
    ...
    \item Funzioni per l'analisi delle immagini: istogrammi, analisi strutturale, descrittori di forme e motion analysis.
    \end{itemize}
\item \textbf{Modulo videoio}: lettura e scrittura di file, nonch\'e analisi di flussi video. Le funzioni principali implementate sono: analisi del movimento, sottrazione del background e tracciamento di oggetti.
\item \textbf{Modulo objdetect}: implementa funzioni e classi per il rilevamento e tracciamento di oggetti all'interno di immagini e flussi video. OpenCV ottiene tutto ci\`o facendo leva sui Haar Feature-based Cascade Classifier e Histogram of Oriented Gradients object detector.
\item \textbf{Modulo ml}: machine learning. La Machine Learning Library (MLL) \`e un insieme di classi e funzioni per la classificazione statistica, regressione e clustering dei dati. La maggior parte degli algoritmi di classificazione e regressione sono implementati come classi C++. Gli algoritmi implementati dal modulo sono:
    \begin{itemize}
...
\noindent La struttura fondamentale della libreria OpenCV \`e l'oggetto \verb|Mat|, il quale \`e usato come contenitore delle immagini. Esso \`e una classe C++ formata da due componenti principali:
\begin{itemize}
\item Un header: contenente informazioni come la dimensione della matrice, il metodo usato per salvarla, l'indirizzo dove \`e salvata e un puntatore alla matrice.
...
\end{itemize}
Questa struttura \`e la stessa per tutti i tipi di immagine, indipendentemente dal formato nella quale \`e salvata. Questa astrazione facilita notevolmente la manipolazione delle immagini in quanto ci si riconduce velocemente ad una matrice di pixel manipolabile. Questa conversione \`e garantita dal modulo imgcodecs, la cui funzione non \`e altro che recuperare i dati sui pixel dell'immagine sulla quale si sta lavorando e generare i metadati per il tipo \verb|Mat|.
...
\begin{itemize}
\item Permettere l'avanzamento della computer vision fornendo non solo una libreria di codice open-source pre-ottimizzato ma anche per costruire l'infrastruttura base di applicazioni di visione artificiale.
\item Condividere conoscenza della stessa fornendo una infrastruttura comune sulla quale tutti gli sviluppatori potessero costruire.
...
\end{itemize}
Ad oggi il supporto al progetto \`e dato dall'organizzazione no-profit OpenCV.org, la quale mantiene un sito per la documentazione e continua a sviluppare e integrare nuovi algoritmi all'interno del progetto. La libreria \`e ormai lo standard de facto per le applicazioni di computer vision e image processing, vista la sua diffusione e la community molto attiva.

Per lo sviluppo dell'applicazione Passenger Counter la scelta di utilizzare questa libreria \`e stata quasi obbligata visto l'ottimo supporto e diffusione. Inoltre la vasta portabilit\`a e l'attenzione alle performance la rendono particolarmente adatta al caso embedded real-time.

%% Interruzione di pagina
\newpage

\subsection{Ambiente di sviluppo per l'OS: Yocto Project}\label{Yocto}
Per integrare nella piattaforma hardware gli strumenti software necessari al funzionamento dell'applicativo \`e stato indispensabile realizzare una distribuzione Linux customizzata adatta alle esigenze del programma. Per farlo \`e stato utilizzato il progetto Yocto, standard industriale de facto per realizzare distribuzioni Linux embedded.

Yocto Project \`e un insieme di strumenti open source finalizzati alla creazione di distribuzioni Linux per sistemi immersi, corredate da toolchain di cross-compilazione ed emulatori, che siano indipendenti dall'architettura hardware. Esso \`e quindi un progetto ombrello sotto il quale si raccolgono vari sotto-progetti volti allo sviluppo di sistemi Linux.

Le architetture supportate dal progetto sono le pi\`u diffuse in questo ambito: ARM, MIPS, PowerPC e x86/x86-64. Una parte fondamentale di questo progetto \`e il build system open source basato sull'architettura OpenEmbedded. Questa implementazione di OpenEmbedded \`e chiamata Poky.

OpenEmbedded \`e un framework software usato per creare distribuzioni Linux embedded. Il build system \`e basato sulle ricette BitBake le quali sono degli script bash specializzati che automatizzano la compilazione e la installazione di pacchetti software. BitBake stesso \`e uno dei componenti fondamentali di Yocto Project: \`e un build automation software, ovvero un tool che legge dei metadati sotto forma di ``ricette'' ed esegue task in base a questi metadati. Le ricette BitBake consistono di un URL che punta alla sorgente del pacchetto software, dipendenze e opzioni di compilazione e installazione. Durante il processo di build sono usate per tenere traccia delle dipendenze ed effettuare la cross-compilazione dei pacchetti affinch\'e sia possibile installarli sulla target distribution.
...
Purtroppo, avendo la scheda una sola porta HDMI disponibile in output, \`e stato necessario procurarsi un modulo aggiuntivo da collegarsi all'header FMC per avere a disposizione una ulteriore porta HDMI in ingresso. Questo modulo \`e il AES-FMC-HDMI-CAM-G HDMI Input/Output FMC prodotto dalla Avnet\cite{avnet:moduloHDMI}.
...
I compilatori del sistema SDSoC infatti analizzano il programma per determinare il data flow tra le funzioni hardware e software e generano un application specific system-on-chip per realizzare il programma. Per raggiungere performance elevate ogni funzione in hardware viene eseguita su un thread indipendente; il compilatore genera i componenti hardware e software che assicurano la sincronizzazione tra i thread hardware e software, mentre abilita la computazione e comunicazione in pipeline. Il codice applicativo pu\`o includere pi\`u funzioni hardware, molteplici istanze di una stessa funzione, e chiamare la funzione hardware da parti diverse del programma.
...
Gli strumenti che si sarebbe andati ad utilizzare sono quelli di cui si \`e parlato nella sezione \ref{TecnologieTesi} a pagina \pageref{TecnologieTesi}: Yocto per la realizzazione della distribuzione Linux, la toolchain di Yocto per la cross-compilazione dei sorgenti, OpenCV per l'image processing.
...
\item \textbf{Completezza}: la libreria AuvizCV, seppur molto performante, mancava di moltissime funzioni rispetto ad una versione completa di OpenCV, la quale poteva essere comodamente installata sul ReliaGate. AuvizCV infatti possiede solamente 50 degli oltre 2500 algoritmi presenti sulla versione completa della libreria.
\item \textbf{Performance}: la CPU Intel \`e molto pi\`u performante dell'ARM presente sulla Zedboard. Siccome sarebbe stato comunque necessario eseguire parte del codice OpenCV sulla CPU, le performance ne avrebbero risentito usando l'ambiente Xilinx.
\item \textbf{Compatibilit\`a}: anche se inizialmente non era previsto l'impiego delle telecamere RealSense, la board Xilinx non sarebbe potuta essere compatibile con queste ultime data la mancanza di una porta USB 3.0 e il supporto software. Vista la mole di dati da trasferire infatti, le telecamere RealSense necessitano di un bitrate molto elevato, che pu\`o essere garantito solamente dallo standard USB 3.0.
\item \textbf{Reperibilit\`a componenti}: come visto nella sezione precedente, la board Xilinx necessitava di un componente aggiuntivo per avere una HDMI in ingresso. Questo modulo \`e risultato molto difficile da reperire e le IP hardware/software a supporto dello stesso obsolete.
\end{itemize}
...
In questo capitolo tratteremo la prima versione realizzata del contatore di passeggeri. Inizialmente descriveremo l'hardware utilizzato, tratteremo le tecniche implementate, quindi passeremo a descrivere la struttura del codice e il suo funzionamento. Infine parleremo dei risultati ottenuti da questa implementazione e le problematiche principali.

Questa prima versione non faceva uso delle telecamere RealSense e presentava alcune criticit\`a che vedremo nel seguito. 

\section{Componenti hardware}\label{HardwarePCN}
Per questa prima implementazione del contatore di passeggeri \`e stata utilizzata una semplice webcam come dispositivo di acquisizione video.
...
La sottrazione del background \`e una tecnica molto utilizzata dell'image processing e della computer vision, nella quale viene estratto il primo piano dell'immagine per rilevare soggetti in movimento all'interno di flussi video catturati da una telecamera statica.
...
Come riassunto in figura \ref{fig:SchemaBackSub} l'obiettivo della sottrazione del background \`e l'estrazione delle regioni in movimento (o Foreground, primo piano) comparando il frame corrente con un modello dello sfondo (BG Model) costruito a partire dai frame precedenti del flusso video. Il prodotto di questa computazione \`e la Foreground Mask, una immagine binaria contenente i pixel che appartengono agli oggetti in movimento nella scena.
...
\begin{enumerate}
\item L'inizializzazione del background: dove viene creato il primo modello dello sfondo.
...
L'algoritmo usato all'interno del codice \`e basato su due paper pubblicati da Z. Zivkovic, \textit{Improved adaptive Gausian mixture model for background subtraction} del 2004 e \textit{Efficient Adaptive Density Estimation per Image Pixel for the Task of Background Subtraction} del 2006.

Esso modella ogni pixel del background con una mistura di distribuzioni gaussiane\cite{misturadistribuzioni} selezionando automaticamente il numero appropriato da utilizzare. I pesi della mistura di gaussiane rappresentano il tempo per il quale quei colori permangono all'interno della scena. I colori che pi\`u probabilmente appartengono al background sono quelli che rimangono per un tempo maggiore all'interno della scena e sono i pi\`u statici.

Tipicamente si ricava una immagine in scala di grigi nella quale i pixel neri rappresentano il background e i pixel bianchi il primo piano. Nel caso dell'algoritmo usato per il Passenger Counter, le ombre vengono evidenziate con un colore grigio. 
...
In figura \ref{fig:EsempioBackSub} si pu\`o notare come le persone vengano correttamente associate alla foreground mask poich\'e evidenziate dal colore bianco e come le ombre associate agli oggetti in movimento siano evidenziate dal colore grigio.
...
Nel caso dell'applicazione del Passenger Counter le regioni di interesse, o blob, sono i passeggeri che si vuole tracciare all'interno del raggio visivo della telecamera. Per andare a rilevare questi ultimi si \`e usata la funzione \verb|findContours()|.
...
Si consideri in ingresso una immagine monocroma, la quale, ad esempio, rappresenta una scena a cui \`e stato sottratto lo sfondo. Il foreground sia rappresentato per mezzo di pixel bianchi mentre lo sfondo con pixel neri. L'algoritmo si compone delle seguenti operazioni.

Anzitutto scannerizza l'immagine per individuare il punto dal quale far partire la sequenza che descrive il poligono. Il punto di partenza \`e rappresentato dalle coordinate del pixel all'interno dell'immagine.
...
Quindi passa ad analizzare gli 8 pixel adiacenti al punto di partenza, la direzione del primo pixel appartenente al bordo del poligono \`e salvata nella sequenza. Si considera un pixel appartenente al bordo dell'immagine se condivide almeno un lato o un vertice con almeno un pixel appartenente allo sfondo (cio\`e un pixel nero). Ripete il procedimento fino a trovare la fine della sequenza, rappresentata dalla coordinata del punto di partenza. A questo punto la sequenza descrive completamente il blob.

Dunque ricomincia la scansione dell'immagine per individuare altri poligoni fino a che anche l'ultimo pixel dell'immagine \`e stato analizzato.

Alla fine di questa operazione si otterranno tante sequenze di interi, le quali rappresentano i blob, quanti sono i poligoni presenti nel foreground.
...
In figura \ref{fig:PCNUML} \`e riportata la struttura del programma. Le finalit\`a dei vari componenti del progetto sono le seguenti:
\begin{enumerate}
...
\item \textbf{main}: implementa l'interfaccia utente per gestire i settaggi run-time dei contatori sfruttando i metodi pubblici resi disponibili dalla classe PCN.
\end{enumerate}
...
Ad ogni frame ricevuto dalla telecamera si aggiornano le coordinate e si resetta la variabile \verb|age| di tutti i passeggeri rilevati. I passeggeri non pi\`u nel raggio d'azione della telecamera, invece, vengono aggiornati con l'ultima posizione conosciuta e ``invecchiano'' ad ogni frame, viene aggiornata cio\`e la variabile \verb|age|. Raggiunta una soglia massima i passeggeri vengono rimossi dalla memoria. Anche in questo caso la permanenza in memoria dei passeggeri non pi\`u rilevati \`e dettata dal frame rate della telecamera. Se viene impostata una \verb|age| massima di 60, e si ha un flusso video a 30 fps, dopo due secondi di assenza dal raggio visivo della telecamera i passeggeri vengono rimossi dal vettore.
...
\item \verb|getTrackColor()|: ritorna il colore col quale tracciare la cronologia delle posizioni del passeggero.
...
\item \verb|updateCoords()|: aggiorna la posizione corrente del passeggero.
...
Si noti che questa rappresentazione dei passeggeri all'interno del contatore \`e stata usata anche dalla versione successiva del Passenger Counter con telecamere RealSense. Per dettagli si veda la sezione \ref{StrutturaRSPCN} a pagina \pageref{StrutturaRSPCN}.
...
\item Implementare il ``nucleo'' del programma, ovvero la parte di codice incaricata di effettuare il conteggio degli attraversamenti della soglia da parte dei passeggeri sfruttando gli algoritmi di cui si \`e parlato in precedenza.
...
\noindent Qui di seguito \`e riportato l'header della classe PCN:
...
\item \verb|pMOG2|: questa struttura \`e definita da OpenCV e implementa l'oggetto che va ad applicare l'algoritmo di sottrazione del background MOG2.
\item \verb|halt|: \`e la variabile booleana che permette di chiudere il loop principale di cattura dei frame. Questa variabile \`e manipolata solamente dal metodo \verb|stop()|.
...
\item \verb|pid|: \`e l'intero che viene aumentato al rilevamento di ogni nuovo passeggero. Permette di assegnare un identificativo univoco a tutti gli oggetti rilevati dal contatore.
\item \verb|passengers|: \`e il vettore che implementa la memoria del programma. Qui vengono salvati i passeggeri tracciati dal contatore come spiegato nella sezione \ref{ClassePassenger}.
...
\item Opzioni: questi booleani permettono di modificare il comportamento del Passenger Counter in real-time.
    ...
    \item \verb|saveVideo|: viene usata dal metodo \verb|count()| per dichiarare e inizializzare le strutture necessarie al salvataggio degli stream video.
    \item \verb|setCalibration|: abilita la calibrazione real-time di alcuni parametri del programma. Questa calibrazione \`e resa possibile dal modulo HighGui di OpenCV il quale permette di aggiungere delle trackbar alle finestre video.
    \end{itemize}
\end{itemize}
...
Come \`e possibile notare dal codice, al costruttore viene passato un intero che identifica la telecamera che verr\`a usata come input dal contatore. Ci\`o permette di istanziare pi\`u contatori contemporaneamente.
...
Nel seguito sono riportati i parametri di impostazione della funzione che implementa l'algoritmo di sottrazione del background:
    ...
    \item \verb|varThreshold|: soglia sulla distanza quadratica di Mahalanobis\cite{distanzaMahalanobis} tra pixel e il modello dello sfondo, atta a discriminare qualora un pixel sia ben descritto dal modello. In caso negativo il pixel viene attribuito al foreground. Si \`e lasciato il valore di default associato a 4 sigma.
...
Si noti che \`e stato necessario utilizzare delle direttive al compilatore per la chiamata alla funzione di creazione del sottrattore del background poich\'e la versione installata sul ReliaGate di OpenCV era diversa da quella presente sulla macchina usata per lo sviluppo dell'applicazione e utilizza una sintassi diversa.
...
\item \verb|count()|: \`e il nucleo del programma. Al suo interno avvengono le operazioni di elaborazioni delle immagini e il conteggio degli attraversamenti. Questo metodo verr\`a analizzato separatamente nella prossima sezione.
\end{itemize}
...
\section{Analisi del nucleo del programma: funzione count()}\label{kernelPCN}
...
\item Rilevamento e tracking dei passeggeri.
...
\noindent Variabili:
\begin{itemize}
\item \verb|color|: contiene i frame dello stream a colori in ingresso dalla telecamera. Rappresenta l'input che si va a manipolare per rilevare gli oggetti in movimento.
\item \verb|fgMaskMOG2|: contiene i frame in uscita dall'algoritmo di sottrazione del background, ovvero la foreground mask.
\item \verb|denoisedMat|: contiene il risultato dell'applicazione delle operazioni di riduzione del rumore sulla foreground mask.
\item \verb|morphTrans|: \`e una copia di \verb|denoisedMat| che verr\`a usata per ricavare i dati di tracciamento. \`E necessario creare una copia per poter visualizzare i risultati di riduzione del rumore prima che il flusso video sia processato ulteriormente.
\end{itemize}
...
\item \verb|whiteThreshold|: soglia utilizzata per la sottrazione delle ombre.
...
\begin{enumerate}
\item Anzitutto si fa affidamento a OpenCV per andare a leggere i frame ricevuti dalla telecamera. Essi vengono direttamente convertiti in tipo \verb|Mat| per poter essere manipolati facilmente dal resto del programma. Tutto questo \`e implementato dal metodo \verb|read()| della classe \verb|VideoCapture|. La matrice \verb|color| va a raccogliere questi dati di ingresso.
\item I frame vengono passati alla struttura che applica la sottrazione del background \verb|pMOG2|. OpenCV prevede la creazione di un oggetto \verb|BackgroundSubtractor| il quale ha tra i suoi metodi l'applicazione della sottrazione dello sfondo (metodo \verb|apply()|). Il risultato di questa operazione \`e la foreground mask, la quale \`e salvata nella matrice \verb|fgMaskMOG2|.
\item Seguono delle trasformazioni sui frame atte a minimizzare il rumore nell'immagine e semplificare il lavoro della funzione di rilevamento dei contorni.
    \begin{enumerate}
    \item Threshold: soglia atta ad eliminare le ombre. Come abbiamo visto in precedenza l'algoritmo di sottrazione del background rileva le ombre e le colora di grigio. \`E possibile eliminarle imponendo una soglia che agisca sul colore dell'immagine. \verb|threshold()| fa in modo che ogni pixel il cui valore numerico \`e superiore al valore dato come soglia (\verb|whiteThreshold|) viene portato al valore \verb|MAXRGBVAL| cio\`e bianco. Ogni pixel che si trova sotto la soglia viene posto a 0, cio\`e nero. Ovvero:
    
    \begin{equation}
    dstPixel(x,y) = \begin{cases} MAXRGBVAL, & \mbox{se } srcPixel(x,y) > threshold \\ 0, & \mbox{altrimenti } \end{cases}
    \end{equation}

    Cos\`\i\ facendo le ombre, che hanno un colore il cui valore RGB \`e inferiore al valore di soglia, vengono portate a zero e non vengono rilevate nei passaggi seguenti, diminuendo cos\`\i\ gli errori nel conteggio.
    ...
    \item Dilatazione: Questa operazione ha come risultato quello di far ``crescere'' le regioni bianche dell'immagine. L'obiettivo \`e quello di rendere pi\`u facilmente rilevabili gli oggetti appartenenti alla foreground mask.
    \item Blur: Questa operazione effettua una sfocatura dell'immagine. L'obiettivo \`e sempre quello di ridurre il rumore per mezzo di un semplice filtro lineare.
    \end{enumerate}
\end{enumerate}
...
\subsection{Rilevamento e tracking dei passeggeri}
...
while(!halt) {
    ...
    for(unsigned int idx = 0; idx < contours.size(); idx++) {
        ...
        if(areaCurrentObject > areaMin) {
            ...
            // Drawing center and bounding rectangle
...
\item Per ogni contorno completo rilevato dalla funzione vengono eseguite le seguenti operazioni:
    \begin{enumerate}
    \item Si calcola l'area per mezzo della funzione OpenCV \verb|contourArea()|. Se supera una certa area minima (\verb|areaMin|) si assume che si tratti di una persona.
    \item Si ricava il rettangolo che inscrive i contorni tramite la funzione \verb|boundingRectangle()|.
    \item Di questo rettangolo viene calcolato il centro \verb|objCenter| grazie ad una semplice operazione geometrica.
    \item Per semplificare lo sviluppo e il debugging si va a disegnare questo rettangolo e il rispettivo centro sull'immagine a colori per mezzo delle funzioni \verb|rectangle()| e \verb|circle()| rese disponibili da OpenCV.
    \end{enumerate}
\end{enumerate}
...
\item L'immagine a destra riporta il frame di ingresso sul quale sono stati disegnati il rettangolo che inscrive il contorno e il suo centro.
\end{itemize}
...
Abbiamo visto che la classe PCN \`e dotata di una variabile privata data da un vettore di \verb|passenger| che rappresenta il database dei passeggeri rilevati dalla telecamera. Il tracciamento di questi avverr\`a tramite il seguente codice.
...
\begin{enumerate}
\item Se il centro dell'entit\`a rilevata si trova entro una certa distanza massima da un passeggero che si trova all'interno del database, si assume che siano lo stesso passeggero. Questa distanza \`e data dalle variabili \verb|xNear| e \verb|yNear| e in figura \ref{fig:NearRect} \`e visualizzato il rettangolo formato da questi due parametri. Alla ricezione di un nuovo frame dalla telecamere, se il nuovo centro dell'oggetto si trova all'interno del rettangolo verde pi\`u scuro, si assume che sia lo stesso oggetto rilevato precedentemente che si \`e spostato.
\item Qualora si trattasse di un passeggero gi\`a presente in memoria se ne aggiorna la posizione per mezzo del metodo \verb|updateCoord()| e resetta il parametro \verb|age|. Nel caso abbia uno storico maggiore di una singola coordinata, si verifica che non abbia superato la soglia virtuale rappresentata dalla met\`a del raggio visivo della telecamera. 

Il superamento della soglia \`e verificato come segue:
    \begin{itemize}
    \item Se la penultima coordinata registrata si trova al di sotto della soglia visiva e l'ultima coordinata sopra, si aggiorna il contatore degli ingressi.
    \item Se la penultima coordinata registrata si trova al di sopra della soglia visiva e l'ultima coordinata sotto, viene aggiornato il contatore delle uscite.
    \end{itemize}
\item Nel caso l'entit\`a non si trovasse vicino a nessun passeggero noto, ovvero non ricade all'interno del rettangolo formato da \verb|xNear| e \verb|yNear| per nessuno dei passeggeri in memoria, si tratta di un nuovo passeggero appena entrato all'interno del raggio visivo della telecamera. Si procede quindi ad aggiornare il database creando  nuovo oggetto appartenente alla classe \verb|passenger| e aggiungendolo in memoria per mezzo del metodo \verb|push_back()|.
\end{enumerate}
...
Qualora nel raggio d'azione della telecamera siano presenti pi\`u passeggeri, bisogna tenere conto del funzionamento dell'algoritmo di sottrazione dello sfondo. Infatti, se due persone sono troppo vicine tra loro (ad esempio spalla a spalla), nella foreground mask figurer\`a un'unica sagoma bianca e i due passeggeri verranno inglobati in un unico blob. Quindi, per rilevare correttamente l'attraversamento di pi\`u persone contemporaneamente, si sfrutta l'area rilevata nei passaggi precedenti: se l'area dell'entit\`a rilevata supera una certa soglia (\verb|max1PassArea|) la si considera un blob formato da due passeggeri. Dunque, al rilevamento dell'attraversamento della soglia, si aggiorna il contatore aggiungendo due attraversamenti invece che uno. Allo stesso modo si procede per il rilevamento di tre passeggeri.

%% Interruzione di pagina
\newpage

Per visualizzare quanto avviene all'interno del programma e semplificare il debugging tramite OpenCV si \`e deciso di plottare a schermo tutte le informazioni utilizzate nella sezione precedente.
...
\item Per ogni passeggero in memoria:
    ...
    \item Visualizza accanto al centro del blob l'identificatore univoco \verb|pid| tramite la funzione \verb|putText()|.
...
\begin{lstlisting}[language=C++, caption=main.cpp, style=customcpp, firstnumber=90]
                case('q'):
...
\item Vengono lanciati in parallelo i contatori per mezzo del metodo \verb|start()|.
...
Nel seguito sono riportati alcuni fotogrammi tratti da una registrazione di un test del contatore. Per il test si \`e utilizzata la telecamera descritta nel paragrafo \ref{HardwarePCN}, la quale \`e stata montata sulla traversa di una porta ad una altezza di circa 2,10 m.

Si \`e deciso di procedere in questo modo per poter simulare al meglio l'ambiente di applicazione finale. Il Passenger Counter Eurotech infatti, da specifiche viene montato circa a 2 metri di altezza sopra il punto di accesso sul quale si vuole effettuare il conteggio\cite{eurotech:DynaPCN10-20}.

Ogni figura riporta a sinistra l'immagine tratta dal video in ingresso dove sono state disegnate le forme per rendere pi\`u comprensibile il tracciamento, al centro l'immagine ottenuta in seguito alla sottrazione del background (cio\`e la foreground mask contenuta in \verb|fgMaskMOG2|), a destra l'immagine cui sono state applicate le operazioni di sottrazione del rumore (cio\`e \verb|denoisedMat|).
...
In figura \ref{fig:PCNTest3} \`e importante notare l'effetto di riduzione del rumore ed eliminazione dell'ombra ottenuti. Nonostante ci\`o l'ombra proiettata lateralmente sul muro, evidenziata tramite il cerchio rosso, viene rilevata erroneamente come oggetto in movimento. Essa viene quindi tracciata dal programma come se si trattasse di un passeggero.

Inoltre, al centro del fotogramma, \`e presente un ulteriore falso tracciamento dovuto ad un picco di rumore registrato dalla telecamera.
...
In figura \ref{fig:PCNTest4} \`e nuovamente possibile notare l'effetto di riduzione del rumore molto elevato in questo frame. 

In questo caso l'ombra proiettata sul muro laterale \`e stata correttamente rilevata come ombra e quindi eliminata dalle operazioni di riduzione del rumore. Nonostante ci\`o nel frame a colori l'ombra sul muro ha lasciato una scia di tracciamento, ci\`o \`e dovuto al fatto che, pur essendo sparita dal raggio visivo, il suo parametro \verb|age| non ha ancora raggiunto il valore massimo e quindi non \`e ancora stata eliminata dalla memoria.

%% Interruzione di pagina
\newpage

\subsection{Performance}
Per la misura dei tempi di esecuzione si \`e fatto uso della libreria standard \verb|chrono| di C++. I test sono stati effettuati usando due telecamere di pari risoluzione ma con diverso framerate su due diversi dispositivi: il ReliaGate 20-25 e una macchina Linux ad alte prestazioni. I tempi sono stati mediati sul numero di cicli effettuati, cio\`e pari al numero di frame catturati dalla telecamera.
...
\begin{figure}[h!]
    ...
    symbolic y coords = {Sottrazione background, Sottrazione rumore, Rilevamento contorni, Tracking passeggeri},
    nodes near coords,
    grid = major,
...
\begin{table}[h!]
    ...
    Rilevamento e tracking dei passeggeri & 0.178409 ms & 0.026478 ms \\ \hline
...
\begin{table}[h!]
    ...
    Rilevamento e tracking dei passeggeri & 0.279599 ms & 0.028581 ms \\ \hline
...
\item \textbf{Tracciamenti indesiderati}: Come visto nella sezione precedente, nonostante l'algoritmo riesca a rilevare la maggior parte delle ombre come tali, spesso queste vengono comunque tracciate erroneamente. Ci\`o comporta errori nel conteggio.
...
L'intero codice della versione del Passenger Counter con sottrazione dello sfondo, nonch\`e le istruzioni per la compilazione e l'esecuzione del programma, sono disponibili pubblicamente all'indirizzo:

\begin{verbatim}
https://github.com/mattdibi/PassengerCounter
\end{verbatim}

\chapter{Passenger Counter con telecamere RealSense}\label{CapitoloRSPCN}
In questo capitolo tratteremo la versione del contatore di passeggeri che utilizza le telecamere RealSense. Inizialmente descriveremo la struttura del codice, gli algoritmi utilizzati e il suo funzionamento. Infine parleremo dei risultati ottenuti da questa implementazione e i problemi principali.
...
\item \textbf{RSPCN}: \`e la classe che implementa il contatore di passeggeri. Questa fa uso della libreria librealsense di cui si \`e discusso nel paragrafo \ref{librealsense} e assolve gli stessi compiti della classe PCN visti nella versione precedente del Passenger Counter.
...
Questa classe ha gli stessi obiettivi della classe PCN nel caso del Passenger Counter a sottrazione del background, con qualche aggiunta. Essa deve infatti, oltre ai compiti visti in precedenza:
...
\item Convertire i dati ottenuti dalle telecamere RealSense in qualcosa di utilizzabile dal resto del programma.
\end{itemize}
...
\noindent Variabili:
\begin{itemize}
\item \verb|dev|: \`e la telecamera dalla quale il contatore raccoglie i dati. Questo tipo \`e definito nella libreria librealsense ed \`e usato per andare a raccogliere i frame in uscita dalla telecamera, nonch\`e altri dati utili per il funzionamento del programma.
\item \verb|cameraDevice|: si \`e resa necessaria per poter distinguere i due modelli di telecamere usati durante lo sviluppo del progetto poich\'e necessitano di settaggi diversi per il loro funzionamento.
...
\item \verb|scale|: \`e il fattore di scala alla quale viene impostata la telecamera, ricavato in run-time.
...
\item Opzioni: questi booleani permettono di modificare le impostazioni del contatore in real-time.
    ...
    \item \verb|saveVideo|: viene usata dal metodo \verb|count()| per dichiarare e inizializzare le strutture necessarie al salvataggio degli stream video.
    \item \verb|setCalibration|: abilita la calibrazione real-time di alcuni parametri del contatore. Questa calibrazione \`e resa possibile dal modulo HighGui di OpenCV il quale permette di aggiungere delle trackbar alle finestre che visualizzano gli stream video.
    \item \verb|framerateStabilizationOn|: va ad agire sulla funzione per mezzo della quale si vanno a ricavare i frame dalla telecamera. Se lasciato \verb|true| viene usato un metodo bloccante che attende l'arrivo dei frame. Altrimenti viene usato un metodo non bloccante, ci\`o comporta un framerate molto pi\`u elevato ma altrettanto meno stabile.
    \end{itemize}
\end{itemize}
...
Una volta inizializzata la variabile \verb|dev| si ricava il tipo di telecamera e, in base al nome, vengono configurati gli stream in ingresso. Ci\`o si \`e reso necessario perch\'e le telecamere operano a risoluzioni e framerate diversi. Si \`e deciso quindi di impostare i flussi come segue:
...
Infine si ricava il fattore di scala al quale opera la telecamera e viene salvato all'interno della variabile \verb|scale|.

\item \verb|start()|: questo metodo crea il thread dove verr\`a eseguito il loop principale del programma. Dopo aver creato il thread lancia il metodo \verb|count()|.
\item \verb|count()|: anche in questo caso rappresenta il cuore del Passenger Counter e verr\`a analizzata separatamente. 
\item \verb|getColorMap()| e \verb|getFrame()| sono le funzioni che convertono i frame forniti dalle telecamere RealSense in qualcosa di utilizzabile. Nel seguito vedremo in dettaglio cosa fanno.
\end{itemize}
...
La sequenza necessaria al funzionamento \`e la seguente.
\begin{itemize}
\item Dichiarazione della variabile della classe \verb|context|: questa classe definita nella libreria librealsense contiene tutti gli handle a tutte le telecamere connesse. \`E necessaria per ricavare il numero di telecamere connesse e assegnare le device ai vari contatori. Si noti che all'interno del sistema di Passenger Counter deve esistere una sola istanza di tipo \verb|context|.
...
Questo metodo si \`e rivelato utile in fase di sviluppo poich\'e fornisce una rappresentazione dei dati in uscita dalla telecamera molto intuitiva. Non ha un ruolo funzionale nel programma ma facilita la comprensione di quanto accade al suo interno, quindi \`e opportuno spiegarne il funzionamento.
...
Come si \`e detto nel paragrafo \ref{librealsense} l'output delle telecamere RealSense \`e dato da un flusso di immagini in scala di grigi i cui pixel hanno formato 16 bit privi di segno, i quali rappresentano la distanza del pixel dalla telecamera. Il valore 0 significa che non si \`e potuti ricavare dati sulla distanza. Questo \`e il formato dell'immagine che ci aspettiamo in ingresso alla funzione e in figura \ref{fig:rawDepthImage} \`e riportato il risultato della visualizzazione per mezzo di OpenCV di questo formato di immagini.
...
Si noti che la leggera discrepanza tra le due immagini \`e dovuta al fatto che il sensore a infrarossi e quello per le immagini a colori si trovano in posizioni diverse. Inoltre il sensore a infrarossi ha un FOV (Field Of View) pi\`u elevato.

\subsection{Funzionamento getColorMap()}
Il primo passo \`e portare il valore dei pixel per i quali non \`e stato possibile ricavare alcun dato di profondit\`a alla massima distanza rappresentabile. Cio\`e si pongono tutti i pixel nulli a 65535 per mezzo della funzione \verb|setTo()|.

Quindi si converte l'immagine da 16 bit a 8 bit usando un fattore di scala. Si noti che questa conversione \`e con perdita.

Si ottiene il risultato in figura \ref{fig:getColorMapStep1}.
...
In ingresso si ha il formato immagine illustrato in figura \ref{fig:rawDepthImage} e la soglia espressa in centimetri da applicare all'immagine.

Come nel caso precedente si applica la funzione \verb|setTo()| per imporre tutti i pixel nulli a 65535. Quindi si converte l'immagine da 16 bit privi di segno a 32 bit floating point per mezzo della funzione \verb|convertTo()|. Questo passaggio  \`e necessario per poter passare l'immagine alla funzione \verb|threshold()|, la quale accetta solamente immagini nel formato 8 bit privi di segno o 32 bit floating point. Si \`e scelto il secondo formato per non perdere informazioni di profondit\`a usando una conversione con perdita. Il risultato di questo passaggio \`e visibile in figura \ref{fig:getFrameStep1}.

Si passa quindi ad applicare la soglia sull'immagine. Siccome i valori dei pixel rappresentano una distanza \`e possibile eliminare tutto ci\`o che si trova oltre una certa lunghezza dalla telecamera per mezzo della funzione \verb|threshold()|. Anzitutto \`e necessario convertire la soglia da centimetri al valore numerico applicabile sui pixel, quindi si passa all'applicazione della soglia vera e propria.

\verb|scale| \`e la scala alla quale \`e impostata la telecamera e viene ricavata interrogandola per mezzo della libreria librealsense. Va moltiplicata per un fattore cento in quanto l'unit\`a di misura con la quale viene fornita sono i metri mentre la soglia \`e espressa in centimetri.

%% Interruzione di pagina
\newpage

\verb|threshold()| va quindi ad effettuare la seguente operazione:
...
Infine, poich\'e si vuole avere che gli oggetti in primo piano siano bianchi e gli oggetti dello sfondo neri, non resta che sottrarre l'immagine ad un matrice delle stesse dimensioni in cui tutti i pixel sono bianchi. Il risultato \`e riportato in figura \ref{fig:getFrameStep3}.
...
\`E stato possibile riutilizzare in parte il codice della versione del contatore a sottrazione del background per le ultime due sezioni. Vi sono per\`o alcune differenze nel codice che vedremo nel seguito.
...
Come nella versione precedente del Passenger Counter, in questa fase avviene l'estrazione degli oggetti in primo piano.

\noindent Variabili:
\begin{itemize}
\item \verb|color|: questa \`e la matrice che contiene i frame dello stream a colori in ingresso dalla telecamera. Verr\`a usata solamente per visualizzare le informazioni sul funzionamento del contatore.
\item \verb|frame|: rappresenta l'output della funzione \verb|getFrame()| di cui si \`e parlato in precedenza.
\item \verb|rawDepth|: questa \`e la matrice che contiene i frame grezzi dello stream di profondit\`a in ingresso dalla telecamera ad infrarossi. \`E una copia della matrice \verb|depth| prima che sia passata alle funzioni di elaborazione delle immagini.
\item \verb|depthColorMap|: rappresenta l'output della funzione \verb|getColorMap()|.
\item \verb|morphTrans|: \`e la copia di \verb|frame| a seguito dell'applicazione delle funzioni di riduzione del rumore.
\end{itemize}
...
\item Quindi si effettua l'operazione di riduzione del rumore per mezzo della funzione \verb|blur()|.
\end{enumerate}
...
A questo punto vengono rilevati i contorni degli oggetti in primo piano tramite la funzione \verb|findCountours()|. Per ogni oggetto cos\`\i\ ricavato se ne calcola l'area, se questa supera un valore minimo deciso arbitrariamente in fase sperimentale viene considerato un passeggero.
...
\item Se il passeggero non si trova vicino a nessun passeggero nel database viene considerato un nuovo passeggero e inserito nel database \verb|passengers|.
\end{itemize}
...
Una volta aggiornate le posizioni viene letto l'intero database dei passeggeri attualmente rilevati all'interno del raggio visivo della telecamera. Per ognuno di essi:

\begin{enumerate}
\item Se ne disegna la traccia dello storico delle posizioni per mezzo della funzione \verb|polylines()|.
\item Si rilevano gli attraversamenti della soglia:
    \begin{itemize}
    \item Se l'ultima posizione si trova al di sotto della soglia e la nuova posizione rilevata al di sopra, viene aggiornato il contatore in ingresso. 
    \item Se l'ultima posizione si trova al di sopra della soglia e la nuova posizione rilevata al di sotto, si aggiorna il contatore in uscita. 
    \end{itemize}
...
\item I passeggeri la cui \verb|age| supera un limite massimo deciso in precedenza, vengono rimossi dalla memoria perch\'e troppo a lungo assenti dal raggio visivo della telecamera.
\end{enumerate}

A differenza del caso del Passenger Counter a sottrazione del background, l'area non viene usata per decidere quante persone hanno effettuato l'attraversamento della soglia. Questo perch\'e, nel caso delle telecamere RealSense, \`e possibile rilevare con precisione la testa dei passeggeri invece dell'intera sagoma.

Come nella versione precedente si \`e deciso di plottare a schermo tutte le informazioni utili a semplificare la comprensione del funzionamento del programma e al debugging. Un esempio della schermata tipica durante l'esecuzione del contatore \`e riportato in figura \ref{fig:RSPCN}.
...
Si noti che la lieve differenza nella posizione delle due telecamere fa in modo che il rettangolo che inscrive l'oggetto tracciato, e cos\`\i\ pure il centro dell'oggetto, siano leggermente spostati rispetto all'oggetto poich\'e il tracking viene effettuato sullo stream proveniente dalla telecamera a infrarossi, mentre il rettangolo viene disegnato sullo stream proveniente dalla telecamera a colori. 
...
Nel seguito sono riportati alcuni fotogrammi tratti da una registrazione di un test del contatore effettuata utilizzando la telecamera Intel RealSense SR300. Il setup del test e il formato dei frame sono gli stessi del caso precedente.
...
A differenza del test precedente nell'immagine contenente i dati di profondit\`a non viene rilevato il pavimento. Questo \`e dovuto al fatto che il range operativo della telecamera SR300 \`e inferiore a quello della R200. Ci\`o per\`o non influisce sul corretto funzionamento del contatore.
...
In questo caso, \`e stata rilevata correttamente l'intera testa del passeggero. Questo \`e dovuto al fatto che la telecamera SR300 ha un range di funzionamento molto migliore poich\'e sfrutta la luce strutturata per ricostruire i dati di profondit\`a.
...
\subsection{Performance}
Per la misura dei tempi di esecuzione si \`e fatto nuovamente uso della libreria standard \verb|chrono| di C++. I test sono stati effettuati usando le due telecamere RealSense su due diversi dispositivi: il ReliaGate 20-25 e una macchina Linux ad alte prestazioni. I tempi sono stati mediati sul numero di cicli effettuati, cio\`e pari al numero di frame catturati dalla telecamera.

Nel seguito sono riportati i risultati relativi alle performance dell'applicazione utilizzando la telecamera RealSense SR300 impostata ad una risoluzione di 640 x 480 pixel a 30 fps.
...
\begin{figure}[h!]
    ...
    enlarge y limits  = 0.2,
    enlarge x limits  = 0.02,
    symbolic y coords = {Metodo getFrame, Sottrazione rumore, Rilevamento contorni, Tracking passeggeri},
    nodes near coords,
    grid = major,
    grid style={dashed,gray!30},
    xlabel = {Tempo di esecuzione [ms]},
...
\begin{table}[h!]
    ...
    Rilevamento e tracking dei passeggeri & 0.033218 ms & 0.054902 ms \\ \hline
...
Nel seguito sono riportati i risultati relativi alle performance dell'applicazione utilizzando la telecamera RealSense R200 impostata ad una risoluzione di 320 x 240 pixel a 60 fps.
...
\begin{table}[h!]
    ...
    Rilevamento e tracking dei passeggeri & 4.48531e-4 ms & 1.29373e-4 ms \\ \hline
    Frame Rate & 60 fps & 60 fps \\ \hline
	\end{tabular}
    \caption{Performance Passenger Counter con telecamera RealSense R200.}
\end{table}

La differenza dei tempi di esecuzione nei due test \`e dovuta al fatto che nel caso della telecamera R200 si \`e usata una risoluzione pi\`u bassa e un framerate pi\`u elevato del caso della RealSense SR300.
...
Si noti inoltre come i tempi di esecuzione del loop principale siano pressoch\'e identici per entrambi i sistemi: ora entrambi impiegano la maggior parte del tempo di CPU ad attendere il frame successivo in ingresso dalla telecamera.

%% Interruzione di pagina
\newpage

\section{Problemi}
Nonostante i miglioramenti anche questa versione del software non \`e esente da problemi.

Anzitutto \`e necessario considerare che il programma \`e molto dipendente dall'altezza dei passeggeri. La soglia applicata alle informazioni di profondit\`a implica che i passeggeri aventi un'altezza inferiore a detta soglia non verranno rilevati, mentre i passeggeri troppo alti potrebbero risultare fuori dal range della telecamera. Siccome si suppone che la telecamera sia posta a circa 2 metri da terra, avendo una soglia massima posta a 45 cm, le persone pi\`u basse di 150 cm risultano invisibili al contatore. Questo problema potrebbe essere risolto applicando una soglia pi\`u ``intelligente'', che tenga conto dei massimi relativi nell'immagine.
...
Per comodit\`a nel seguito verranno adottate le seguenti abbreviazioni:
\begin{itemize}
...
\section{Prestazioni}
Finora sono state trattate nel dettaglio le due diverse implementazioni del Passenger Counter. Ora confronteremo quali differenze comportano le diverse tecnologie e algoritmi utilizzati del punto di vista delle prestazioni del programma.

\subsection{Costo computazionale}
L'algoritmo di sottrazione dello sfondo oltre ad essere impreciso \`e molto pesante dal punto di vista computazionale. L'aver spostato gran parte del calcolo sugli ASIC presenti nelle telecamere RealSense permette di avere uno stream in ingresso con un framerate molto pi\`u stabile ed elevato, di conseguenza \`e possibile utilizzare pi\`u telecamere contemporaneamente.

In figura \ref{fig:GraficoTempiPCNRSPCN} a pagina \pageref{fig:GraficoTempiPCNRSPCN} \`e riportato un confronto sui tempi di esecuzione delle funzioni principali delle due implementazioni del contatore. Per il confronto si \`e usata la telecamera SR300 e la webcam Logitech poich\'e posseggono la stessa risoluzione e lo stesso framerate.
...
\begin{figure}[h!]
    ...
    enlarge y limits  = 0.2,
    enlarge x limits  = 0.02,
    symbolic y coords = {Rilevamento passeggeri, Sottrazione rumore, Rilevamento contorni, Tracking passeggeri},
    nodes near coords,
    grid = major,
    grid style={dashed,gray!30},
    xlabel = {Tempo di esecuzione [ms]},
...
\item Ambiente: la telecamera \`e stata montata sulla traversa di una porta ad una altezza di circa 2,10 m. Entrambe le versioni del programma sono state eseguite sul ReliaGate 20-25.
...
Le prestazioni del Passenger Counter a sottrazione del background sono di gran lunga peggiori dell'implementazione successiva. Risulta evidente che il problema maggiore del PCN sono i falsi positivi nel tracciamento.
...
Il fatto che nel RSPCN venga tracciata solo la testa dei passeggeri permette un tracking molto pi\`u preciso. Infatti nel caso del PCN la forma del blob varia continuamente e, poich\'e il tracciamento \`e riferito al centro del blob, ci\`o comporta spostamenti repentini. In figura \ref{fig:trackPCN} possiamo notare come la traccia lasciata dal passeggero sia ricca di discontinuit\`a e linee spezzate, d'altra parte in figura \ref{fig:trackRSPCN} la linea di tracciamento risulta molto pi\`u continua.
...
Appare evidente che l'RSPCN \`e completamente esente da falsi positivi nel tracciamento dovuti alle ombre, mentre nel caso del PCN sono presenti ben due falsi positivi dovuti ad esse. Questo accade nonostante l'algoritmo per la sottrazione del background sia stato pensato per rilevarle e quindi permetterne l'eliminazione.
...
Il codice di cui si \`e parlato finora \`e riferito a questa versione dell'applicativo. Siccome OpenCV e la libreria RealSense sono scritte in questo linguaggio si \`e pensato di usare questo come punto di partenza. In seguito alla prima implementazione si \`e reso necessario il porting negli altri linguaggi per le motivazioni descritte in precedenza.

\subsection{Wrapping in Java}
Come primo passo si \`e deciso di effettuare un wrapping in Java del codice nativo sfruttando il tool open source \textbf{SWIG}\footnote{Pagina web del progetto: \url{http://www.swig.org/}.}.

% TODO: Il seguente paragrafo \`e stato copiato spudoratamente. Cambiare.
SWIG (Simplified Wrapper and Interface Generator) \`e un wrapper open source utilizzato per collegare i programmi o librerie scritte in C o C++ con altri linguaggi come C Sharp, Java, JavaScript, Go, Modula-3, OCaml, Octave, e Scheme.

L'obiettivo \`e quello di consentire la chiamata di funzioni native (C o C++) da altri linguaggi di programmazione, passando tipi di dati complessi, mantenendo la memoria libera, ereditariet\`a delle classi dei vari linguaggi, ecc. Al programmatore \`e delegato il compito di scrivere un file di interfaccia che contenga un elenco di funzioni C/C++ che devono essere rese visibili ad un interprete. SWIG compila il file di interfaccia e genera normale C/C++ e il codice nel linguaggio di programmazione di destinazione. Quindi genera il codice di conversione o serializzazione per le funzioni con argomenti semplici; il codice di conversione per i tipi di argomenti complessi deve essere scritto dal programmatore. Lo strumento crea codice sorgente che fornisce il collante tra C/C++ e il linguaggio di destinazione, a seconda del linguaggio software, questo collante \`e disponibile in due forme:

\begin{itemize}
\item una libreria condivisa che un interprete esistente pu\`o collegare come una qualche forma di modulo di estensione.
...
La versione del Passenger Counter realizzata con questo tool conserva le prestazioni del codice nativo pur permettendo l'interfacciamento con Java. Si \`e comunque ritenuto necessario compiere un ulteriore passo ed effettuare il porting vero e proprio dell'applicazione in Java.

\subsection{Versione Java}
Il porting dell'applicazione ha comportato fin da subito dei problemi di compatibilit\`a dato che, bench\'e OpenCV avesse recentemente introdotto il support a Java, \`e stato necessario utilizzare un wrapper third-party di OpenCV poich\'e lo richiedeva il wrapper ufficiale della libreria librealsense. Entrambi questi wrapper sono stati realizzati dal progetto \textbf{JavaCPP}\footnote{Repository del progetto all'indirizzo: \url{https://github.com/bytedeco/javacpp}.}.

JavaCPP \`e un progetto open source il cui obiettivo \`e fornire un accesso efficiente al codice nativo C++ dentro Java. Esso sfrutta le similitudini sintattiche e semantiche tra Java e C++ per raggiungere questo obiettivo. Inoltre \`e basato sulla Java Native Interface (JNI) quindi funziona per tutte le implementazioni di Java SE.

Praticamente, parsando il codice nativo degli header file delle librerie, realizza delle interfacce Java che permettono al codice Java di effettuare le chiamate delle librerie C++ installate sulla macchina host. Tramite questa pratica, i manutentori del progetto hanno prodotto delle interfacce complete per OpenCV, FFmpeg, libdc1394, PGR FlyCapture, OpenKinect, videoInput, ARToolKitPlus, e altri come parti del progetto \textbf{JavaCPP Presets}\footnote{Repository del progetto all'indirizzo: \url{https://github.com/bytedeco/javacpp-presets}.}.

JavaCPP Presets \`e un sotto progetto di JavaCPP che rende disponibile le configurazioni e le classi di interfacciamento verso le librerie C++ pi\`u usate sfruttando JavaCPP. I file di configurazione nel pacchetto org.bytedeco.javacpp sono usati dal Parser per creare dai file header C/C++ le interfacce Java che puntano al pacchetto di JavaCPP, il quale a sua volta \`e usato dal Generator e il compilatore nativo C++ per produrre le librerie JNI richieste.

Nella repository del progetto sono inoltre disponibili i file \verb|.jar| precompilati per l'interfacciamento alle librerie librealsense, i quali sono stati usati per facilitare il porting dell'applicazione RSPCN in Java.

Purtroppo questa interfaccia fa uso del progetto \textbf{JavaCV}\footnote{Repository del progetto all'indirizzo: \url{https://github.com/bytedeco/javacv}.}, il quale \`e un wrapper Java di OpenCV realizzato sempre dal progetto JavaCPP sfruttando i preset resi disponibili da JavaCPP Presets. \`E stato quindi necessario utilizzare JavaCV al posto di OpenCV per realizzare questa versione del codice nonostante quest'ultimo disponesse di un suo interfacciamento a Java. \`E necessario notare per\`o che il progetto JavaCV \`e stato rilasciato molto prima dell'attuale wrapper Java ufficiale di OpenCV e risulta pi\`u completo della controparte. Nonostante ci\`o le differenze tra i due erano cos\`\i\ spiccate che \`e stata necessaria una revisione abbastanza corposa del codice e in alcuni casi non \`e stato possibile portare tutte le feature presenti nella versione C++.
...
\begin{table}[h!]
    ...
    Rilevamento e tracking dei passeggeri & 4.48531e-4 ms & 0.059985 ms \\ \hline
    Frame Rate & 60 FPS & 60 FPS \\ \hline
	\end{tabular}
...
\begin{enumerate}
\item \textbf{Poky}: la reference distribution del progetto Yocto. Fa da template per la costruzione della propria distribuzione Linux customizzata. Nel progetto si \`e usata la versione 2.2 (nome in codice Morty) a causa delle restrizioni dovute alle librerie RealSense.
...
Come spiegato nella sezione \ref{Yocto} a pagina \pageref{Yocto}, il progetto Yocto \`e basato su ``ricette''. Queste ricette contengono dei metadati con le istruzioni su come installare il software nella distribuzione da realizzare. Il build system esegue, per ogni ricetta, le seguenti operazioni:
...
Questa directory include una copia di BitBake per facilitare l'uso del build system. Solitamente questa copia \`e riferita alla versione pi\`u recente di BitBake. Quando viene eseguito il comando \verb|bitbake| che lancia il processo di build, viene chiamato l'eseguibile principale di BitBake, il quale risiede nella cartella \url{bitbake/bin/}. 

Lanciare l'environment setup script (esempio: \verb|oe-init-build-end| o \verb|oe-init-build-env-memred|) inserisce gli script e la directory \url{bitbake/bin} nel \verb|PATH| della shell.

\subsubsection{build}
Questa cartella rappresenta la Build Directory.

Essa contiene i file di configurazione dell'utente e l'output generato dal build system OpenEmbedded nella sua configurazione standard. La Build Directory viene creata inizialmente quando viene lanciato l'environment setup script di OpenEmbedded.

Le preferenze circa il software da installare nella distribuzione e il processo di build vanno specificate nei due file di configurazione della Build Directory:
...
La sorgente del file local.conf.sample dipende dalla variabile \verb|$TEMPLATECONF|, la quale \`e impostata di default a \url{meta-poky/conf} quando il processo di build proviene dall'ambiente di sviluppo del progetto Yocto, e a \url{meta/conf} quando il processo di build proviene dall'ambiente di sviluppo di OpenEmbedded Core.
...
La sorgente del file bblayers.conf.sample dipende dalla variabile \verb|$TEMPLATECONF|, la quale \`e impostata di default a \url{meta-poky/conf} quando il processo di build proviene dall'ambiente di sviluppo del progetto Yocto, e a \url{meta/conf} quanto il processo di build proviene dall'ambiente di sviluppo di OpenEmbedded Core.
\end{itemize}
...
Qui sono contenuti vari script di integrazione che implementano funzionalit\`a extra nell'ambiente di sviluppo del progetto Yocto (ad esempio: script QEMU). Gli environment setup script aggiungono questa cartella al \verb|PATH| della shell.

\subsubsection{oe-init-build-env}
Questo script \`e uno dei due environment setup script dell'ambiente OpenEmbedded. L'altro script non \`e stato usato nel corso dello sviluppo e non verr\`a trattato nel seguito. Per approfondimenti si faccia riferimento al manuale del progetto Yocto\cite{yocto:megamanual}.

L'esecuzione di questo script tramite il comando \verb|source| in una shell applica cambiamenti al \verb|PATH| e imposta altre variabili fondamentali di BitBake basandosi sulla directory di lavoro corrente. \`E necessario eseguire lo script prima di poter eseguire qualsiasi comando BitBake. Esso sfrutta altri script presenti nella cartella \url{scripts/} per fare gran parte del lavoro.
...
Lo script ricava la lista dei default target dal file conf-notes.txt, il quale si trova nella directory \url{meta-yocto/}.

Eseguire questo script senza una Build Directory ne implica la creazione all'interno dell'attuale working directory. Se invece viene fornita una Build Directory come argomento quando viene eseguito lo script, l'OpenEmbedded build system crea la cartella scelta. Ad esempio, il seguente comando crea una Build Directory chiamata \url{mybuild} che si trova fuori dalla Source Directory.
...
Lo sviluppo della distribuzione \`e stato parallelo allo sviluppo dell'applicazione del contatore. Mano a mano che si aggiungevano componenti aumentavano i requisiti dell'applicativo e di conseguenza venivano aggiunti nuovi tool software alla distribuzione. Ci\`o significava aggiungere gradualmente nuovi layer e nuove ricette al build system.
...
La prima versione \`e stata realizzata per supportare l'applicazione del Passenger Counter a sottrazione del background, quindi prevedeva solamente l'installazione di OpenCV sul ReliaGate 20-25. Per questa versione sono stati aggiunti soltanto due layer alla configurazione standard di Poky:

\begin{itemize}
\item \textbf{meta-intel}: layer contenente metadati per il supporto dell'hardware Intel. Questo layer \`e necessario per poter realizzare un'immagine dell'OS finalizzata all'esecuzione sul processore Intel, sul quale \`e basato il ReliaGate 20-25.
...
Purtroppo questo layer risultava essere compatibile solamente con la versione 4.8 del kernel Linux, contenuta nella versione Morty di Poky. \`E stato quindi necessario effettuare un downgrade della versione di Poky con conseguente downgrade di OpenCV. In questa versione della distribuzione erano quindi presenti OpenCV 3.1 e la libreria librealsense 1.12.1.
...
Questa versione \`e stata realizzata per supportare il Passenger Counter con telecamere RealSense e le varie versioni Java. Ci\`o ha significato installare un Java Run-time Environment e un JDK (Java Developer Kit). Si \`e scelto di installare OpenJDK-8 perch\'e il processo di installazione era pi\`u semplice: sono infatti noti dei problemi di installazione per la versione Oracle del JDK.

\`E stato quindi necessario installare un nuovo layer: \textbf{meta-java}. Purtroppo il versioning di quest'ultimo non ha adottato lo standard dei restanti layer e mostrava problemi di compatibilit\`a con gli altri. \`E stato necessario percorrere all'indietro lo storico dei commit della repository contenente il layer, fino a trovare una versione compatibile con l'immagine realizzata fino a quel punto.
...
\begin{itemize}
\item \verb|MACHINE|: Specifica la target device per la quale l'immagine viene generata. La variabile corrisponde ad un file di configurazione della macchina con lo stesso nome, attraverso il quale le configurazioni specifiche per quella macchina vengono impostate. Di conseguenza, quando \verb|MACHINE| \`e impostato a \verb|quemux86| esister\`a un file qemux86.conf corrispondente nella directory \url{meta/conf/machine}. Nel nostro caso stiamo generando una distribuzione per una architettura Intel 64-bit e il file di configurazione della macchina si trova nel layer \textbf{meta-intel}.
\item \verb|DISTRO|: Il nome della distribuzione. Questa variabile corrisponde ad un file di configurazione di distribuzione la cui radice del nome \`e la stessa dell'argomento della variabile e la cui estensione \`e .conf. Per esempio, il file di configurazione della distribuzione Poky \`e poky.conf e risiede in \url{meta-poky/conf/distro}.
\item \verb|PACKAGE_CLASSES|: specifica il package manager che viene usato dal build system di OpenEmbedded quando vengono creati i pacchetti del software in fase di build. I possibili valori sono: ``\verb|package_rpm|'', ``\verb|package_deb|'' e ``\verb|package_ipk|''.
\item \verb|SDKMACHINE|: La macchina per la quale \`e costruita la toolchain di cross-sviluppo. In altre parole, il SDK \`e costruito in modo tale che possa essere eseguito su un target specifico con il valore \verb|SDKMACHINE|. Il valore punta ad un file .conf corrispondente sotto la cartella \url{conf/machine-sdk/}. Possono essere usati ``\verb|i686|'' e ``\verb|x86_64|'' come possibili valori per questa variabile.
...
\item \verb|BB_DISKMON_DIRS|: Attiva il monitoraggio dello spazio su disco e gli inode disponibili durante la build permettendo di controllare il processo di generazione dell'immagine per mezzo di questi parametri. Ad esempio la dicitura ``\verb|STOPTASKS, ${TMPDIR}, 1G, 100K|'' significa che il processo di build verr\`a fermato dopo il completamento di tutte le task in esecuzione, qualora lo spazio minimo su disco nella directory \verb|${TMPDIR}| scenda sotto 1 GB.
...
\item \verb|PARALLEL_MAKE|: Opzioni extra passate al comando \verb|make| durante la task \verb|do_compile| in modo tale da specificare la compilazione parallela sulla macchina del build system. Questa variabile solitamente \`e nella forma ``\verb|-j x|'', dove x rappresenta il numero massimo di thread paralleli che il comando make pu\`o istanziare.
\item \verb|IMAGE_INSTALL|: Specifica i pacchetti da installare in una immagine, le cui ricette devono trovarsi nei layer specificati in bblayers.conf.
...
\noindent La keyword \verb|require| indica quale ricetta \`e necessaria per l'installazione della libreria.
...
Praticamente, grazie a queste modifiche, se durante il processo di build BitBake rileva che l'immagine possiede un JDK, effettua le operazioni necessarie a generare le API Java di OpenCV.

La patch si \`e resa necessaria in quanto questa operazione, nella versione originale della ricetta, avveniva solamente nel caso in cui BitBake rilevasse il JDK di Oracle, ma per problemi di compatibilit\`a \`e stato possibile installare solamente il JDK OpenJDK-8.
...
Nonostante questi problemi la distribuzione possiede tutti i tool necessari a supportare l'applicativo sviluppato nel corso della tesi, ed \`e ritenuta un ottimo proof of concept da parte di Eurotech.

\chapter{Manuale d'uso del progetto}\label{CapitoloManuale}
In questo capitolo \`e riportata la struttura del progetto e come replicare i risultati ottenuti nel corso dello sviluppo.
...
\begin{itemize}
\item \verb|build_config|: file di configurazione del progetto Yocto e le istruzioni per generare la distribuzione per il ReliaGate 20-25.
\item \verb|cpp_src|: sorgenti per la versione C++ del codice del RSPCN.
\item \verb|java_wrap|: file di configurazione di SWIG per generare il wrapper Java del progetto.
\item \verb|java_src|: sorgenti e file .jar necessari per la versione Java dell'applicativo.
\end{itemize}
...
Inoltre \`e necessario assicurarsi di avere delle porte USB 3.0 disponibili per il collegamento con le telecamere RealSense e un hardware sufficientemente prestante.
...
Come descritto nel paragrafo \ref{ModRicettaOpenCV} \`e necessario modificare la ricetta di OpenCV, il cui percorso \`e:
...
Esiste un problema noto con alcune vecchie versioni del compilatore C++ che possono interrompere il processo di build dell'immagine. Nel caso si presenti l'errore ``\textbf{unrecognized command line option -fno-lifetime-dse}'' \`e necessario che alla ricetta presente al percorso

\begin{verbatim}
$INSTALL_DIR/poky/meta-java/recipes-core/openjdk/openjdk-8-common.inc
\end{verbatim}

\noindent siano applicate le seguenti modifiche presso la linea 224 del file.
...
In questo modo sar\`a possibile visualizzare sulla macchina host gli stream catturati dalle telecamere collegate alla macchina target e il risultato della computazione.
...
\begin{itemize}
\item Senza argomenti: si interfaccia a tutte le telecamere collegate alla macchina e cattura gli input stream allocando un contatore per ogni telecamera.
...
\item \verb|C|: Abilita/Disabilita le trackbar per effettuare la calibrazione dei settaggi del contatore, tra queste impostazioni sono disponibili:
    \begin{itemize}
...
\item \verb|f|: Abilita/Disabilita la finestra nella quale viene visualizzato lo stream dei dati di profondit\`a post processati e che vengono passati all'algoritmo di rilevamento dei contorni (contenuto nella variabile \verb|frame|).
...
\item \verb|0-5|: Seleziona i preset per la telecamera da utilizzare. I preset sono delle impostazioni della telecamera ad infrarossi che ne modificano il comportamento permettendo di impostare l'esposizione, la precisione e altri parametri.
\end{itemize}
...
Qui di seguito \`e riportato uno screenshot della schermata tipica di funzionamento del programma su una macchina host. Il programma si trova in modalit\`a di esecuzione senza argomenti e sono stati visualizzati lo stream a colori e abilitata la calibrazione dei parametri del contatore.
...
\subsubsection{Compilazione ed esecuzione della versione per la macchina host}
Per semplificare la compilazione sono stati resi disponibili degli script il cui uso \`e riportato nel seguito.
...
\subsubsection{Compilazione ed esecuzione della versione per il ReliaGate 20-25}
Dopo aver installato la toolchain di cross-compilazione generata dal progetto Yocto \`e necessario eseguire:
...
Una volta connessi al ReliaGate 20-25 e aver trasferito l'intera cartella di output sul dispositivo target \`e possibile eseguire il programma per mezzo dei comandi:
...
La modalit\`a bare metal e la relativa modalit\`a di registrazione si sono rese necessarie poich\'e, essendo le finestre di questa versione del contatore basate sul framework Java e non su X11, la visualizzazione degli stream video impedirebbe il corretto funzionamento del programma sul ReliaGate 20-25.
...
\item \verb|p|: Selezione i preset per la telecamera da utilizzare.
...
\item \verb|b|: Imposta la quantit\`a di offuscamento dell'immagine per ridurre il rumore (\verb|blur()|).
...
Inizialmente ci si \`e dedicati all'indagine sullo stato attuale delle tecnologie disponibili per l'elaborazione delle immagini in real-time in ambito embedded, considerando al contempo l'hardware disponibile per lo sviluppo, e di studio degli strumenti disponibili. In questa fase \`e stato necessario prendere confidenza con la libreria OpenCV, l'ambiente di sviluppo Xilinx e il progetto Yocto, per poter capire quali fossero i tool pi\`u adatti all'obiettivo che ci si prefiggeva. 
...
Come visto nel paragrafo \ref{ProblemiPCNEurotech} a pagina \pageref{ProblemiPCNEurotech} il contatore realizzato da Eurotech presentava delle criticit\`a che mi \`e stato chiesto di risolvere durante lo sviluppo di questa versione del Passenger Counter. Queste sono state risolte come segue.
...
\begin{itemize}
\item \textbf{Implementazione di algoritmi pi\`u sofisticati per il rilevamento dei passeggeri}: l'implementazione realizzata in questa tesi \`e sicuramente un buon proof of concept ma per una applicazione nel mondo reale \`e necessario implementare un rilevamento dei passeggeri pi\`u solido. Una soluzione interessante sarebbe quella di adottare algoritmi di machine learning per il riconoscimento delle sagome dei passeggeri che transitano sotto la telecamera. Questo permetterebbe una maggior precisione diminuendo il numero di falsi positivi rilevati dalla telecamera.
...
\item \textbf{Porting in OSGi}: l'infrastruttura Eurotech \`e realizzata interamente seguendo le specifiche OSGi e sarebbe opportuno completare il porting del codice del progetto adattandolo a queste specifiche.
\end{itemize}

Purtroppo non \`e stato possibile applicare questi miglioramenti in quanto \`e stata data la precedenza ad altri aspetti del progetto che hanno sottratto tempo e risorse, quindi sono lasciati ad implementazioni future.

%% Interruzione di pagina
\newpage

\section{Risultati distribuzione Yocto}
Anche in questo caso i risultati ottenuti sono stati pi\`u che soddisfacenti. Uno degli obiettivi della distribuzione era dimostrare che il RealiaGate pu\`o supportare librerie e tool software di largo uso come OpenCV e hardware specializzato come le telecamere RealSense. Questo permette di aumentare l'appetibilit\`a del gateway sul mercato e ne amplia gli ambiti di utilizzo in un'ottica non limitata solamente all'applicazione nel sistema del Passenger Counter.
...
\noindent In figura \ref{fig:overviewStereoVision} sono riportati i passaggi fondamentali per ricavare le informazioni 3D.

\begin{enumerate}
\item \textbf{Rettificazione}: le due immagini ottenute dalle telecamere vanno proiettate su un piano comune per permettere la comparazione tra le due. Questo passaggio \`e noto come image rectification, ed \`e necessario a semplificare il passaggio di stereo correpondance.
...
\begin{minipage}[b]{8.5cm}
  \centering
  \includegraphics[width=8cm]{left.png}
  \caption{Immagine telecamera sinistra.}
  \label{fig:leftimage}
\end{minipage}
\ \hspace{2mm} \hspace{3mm} \
\begin{minipage}[b]{8.5cm}
 \centering
  ...
  \caption{Immagine telecamera destra.}
  \label{fig:rightimage}
\end{minipage}
\end{figure}

Si abbiano due immagini ricavate da altrettante telecamere disposte sullo stesso piano ad una distanza nota tra di loro. La tecnica di simple block matching consiste nel prendere una piccola regione di pixel nell'immagine proveniente dalla telecamera sinistra (ad esempio) e cercare la regione pi\`u simile a quella considerata nell'immagine proveniente dalla telecamera destra.

Per esempio, si consideri la regione di pixel contenuta nel riquadro nero nell'immagine proveniente dalla telecamera sinistra, chiameremo questa regione ``riquadro template''.
...
Prima di calcolare la disparity map le due immagini vengono convertite in scala di grigi cosicch\`e si abbiano solo valori compresi tra 0 e 255 per ogni pixel.

Per calcolare la somma delle differenze assolute tra la il riquadro template e la regione di interesse, si sottrae ogni pixel nel riquadro template dal pixel corrispondente nella regione e si prende il valore assoluto di questa differenza. Quindi vengono sommate tutte le differenze il risultato di questa operazione \`e un singolo valore che approssima la misura di similitudine tra le due regioni delle immagini. Un valore basso significa che i riquadri sono molto simili.
...
Per trovare la regione pi\`u simile al riquadro template, si calcola il SAD tra il template e ogni blocco nella regione di ricerca, quindi si sceglie il riquadro con il valore di SAD pi\`u basso.
...
Si noti come nel simple block matching si \`e cercata la corrispondenza tra blocchi solamente in orizzontale e non in verticale. Ci\`o \`e reso possibile dall'operazione di rettificazione delle immagini. Questa operazione consiste nel rilevare un insieme di keypoint, o punti di interesse, all'interno dei due piani, per mezzo di algoritmi di keypoint detection come SIFT o SURF, e quindi applicare delle trasformazioni sulle immagini in modo tale che questi punti siano allineati verticalmente. Ci\`o permette di eliminare eventuali rotazioni o deformazioni dovute alle lenti nelle immagini e si pu\`o ignorare la componente verticale nella ricerca dei riquadri simili durante l'operazione di simple block matching.

\section{Triangolazione}
Si supponga di avere due telecamere disposte sullo stesso piano, poste ad una distanza nota B di cui si conosce la distanza focale f. Dalle immagini catturate si vuole ricavare la distanza tra le due telecamere e un oggetto P.

\begin{figure}[h!]
  \centering
  \includegraphics[scale=0.65]{SchemaStereoVision.png}
...
\noindent In figura \ref{fig:SchemaStereoVision} abbiamo che:
...
\noindent Quindi, sfruttando il fatto che i triangoli PO\ped{R}O\ped{T} e Ppp' sono triangoli simili, possiamo ricavare:
...
\noindent Dove si definisce \textit{disparit\`a}:

\begin{equation}
d \overset{\Delta}{=} x_R - x_T
\end{equation}
...
Vi sono molte informazioni relative alla profondit\`a contenute all'interno dei pattern di luce osservati. Il discostamento di ogni singola banda pu\`o essere direttamente convertito in coordinate 3D. A questo scopo, le bande vengono identificate individualmente tramite diversi metodi.
...
In molte implementazioni pratiche di questa tecnica vengono effettuate una serie di misure che combinano la pattern recognition, identificazione delle bande e trasformate di Fourier per ottenere una ricostruzione non ambigua delle forme in 3D.
