...
\item Realizzare una nuova versione del Passenger Counter puntando a migliorare le prestazione e abbattere i costi, che fosse compatible con le tecnologie usate dall'azienda per i suoi prodotti. 
...
\item Seguono delle trasformazioni sui frame atte a minimizzare il rumore nell'immagine e semplificare il lavoro della funzione di rilevamento dei contorni.
    ...
    Cos\`\i\ facendo le ombre, che hanno un colore il cui valore RGB \`e inferiore al valore di soglia, vengono portate a zero e non vengono rilevate nei passaggi seguenti, diminuiendo cos\`\i\ gli errori nel conteggio.
...
\item \verb|DISTRO|: Il nome della distribuzione. Questa variabile corrisponde ad un file di configurazione di distribuzione la cui radice del nome \`e la stessa dell'argomento della variabile e la cui esensione \`e .conf. Per esempio, il file di configurazione della distribuzione Poky \`e poky.conf e risiede in \url{meta-poky/conf/distro}.
...
\item \textbf{Porting in OSGi}: l'infrastruttura Eurotech \`e realizzata interamente seguento le specifiche OSGi e sarebbe opportuno completare il porting del codice del progetto adattandolo a queste specifiche.
\end{itemize}
